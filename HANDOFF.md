...
**HEAD:** `0fae3e4` · 2026-08-03 · 워킹트리 깨끗
**테스트:** 243개 전부 통과 (`npm test`, 약 13초)
...
| **관리자 승인 절차** | 부장님 | 화면에 "승인 절차는 아직 없습니다"라고 표시만 — 일부러 안 만듦, 아래 참고 |
| **노쇼 처리** | 교수님 | 없음 — 일부러 안 만듦, 아래 참고 |
...
### 관리자 승인 절차·노쇼 처리를 일부러 안 만든 이유

둘 다 "코드가 없다"가 아니라 "룰이 없어서 코드를 못 짠다"에 가깝습니다.

**관리자 승인 절차**를 지금 만든다는 건 곧 호스팅 자체를 승인 뒤로 막는다는
뜻입니다 — 그러면 파일럿 첫날 아무도 상을 못 엽니다. 누가 승인하는지,
기준이 뭔지, 처리 시간이 얼마나 걸리는지가 먼저 정해져야 하고, 그 답에
따라 완전히 다른 기능이 나옵니다(대기 상태 화면 vs 그냥 사후 인증
뱃지처럼 지금 방식 유지).

**노쇼 처리**는 정말 아무 결과 없는 "노쇼 표시" 버튼만 만들면, 이 저장소가
`profile.js`의 `RESTRICTIONS` 주석에서 이미 못박은 원칙(*"a preference the
app never acts on is decoration"*)을 스스로 어기는 셈입니다 — 표시만 하고
아무 결과가 없는 기능은 장식이지 답이 아닙니다. 연락처 교환을 요구할지,
반복되면 어떻게 할지 같은 실제 결과를 정해야 의미가 생깁니다.

두 개 다 코드량이 문제가 아니라 사람이 결정할 몫이라 미뤘습니다.
