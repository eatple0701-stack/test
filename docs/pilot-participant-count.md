...
먼저 보세요. 세 숫자는 서로 다른 것을 셉니다.

| 숫자 | 무엇인가 | 무엇이 아닌가 |
|---|---|---|
| **253** | `profiles` 전체 행 수. 앱을 연 브라우저 수에 가깝습니다 | **참가자 수가 아닙니다.** 열어보고 나간 익명 세션과 개발 접속이 대부분입니다 |
| **10** | 동의 게이트를 통과한 프로필(`rules_version is not null`). `2026-09-02a` 마이그레이션의 `backfilled_rows` 합격값 | 사람 수가 아닙니다 — 개발 접속 계정이 섞여 있습니다(§7) |
| **10 − 제외 목록** | KF 보고서의 **"동의 문구 개정, 재동의 대상 N명"**. 쿼리는 §7 마지막 | 이용자 수가 아닙니다. 동의를 누른 계정 수입니다 |
...
세 숫자를 나란히 놓은 표는 §0에 있습니다. 여기서는 뒤의 두 개만 다룹니다.
