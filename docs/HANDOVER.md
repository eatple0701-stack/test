...
| | |
|---|---|
| Automated tests | **1071, all passing** (`npm test`) |
...
거짓을 지우는 일과 없는 한국어를 새로 쓰는 일은 다른 일이라서 승인도 검증도
따로 받는 게 맞습니다.

### 무엇이 없나

`index.css:11736`이 이렇습니다:

```css
[data-locale="ko"] [class$="__en"],
[data-locale="ko"] [class$="-en"] { display: none; }
```

한국어 설정에서 `__en`으로 끝나는 슬롯이 통째로 사라집니다. 짝이 되는
`__kr` 슬롯에 한국어가 있으면 정상 동작이지만, **한국어 쪽이 짧은 라벨뿐이고
문장이 `__en`에만 있으면 한국어 독자는 문장을 아예 못 봅니다.** 영어가 새는
문제가 아니라 내용이 없는 문제라, 화면을 열어봐도 비어 보이지 않습니다 —
그냥 짧습니다.

확인된 것: **이용 방법 세 단계** (`src/content/howItWorks.js`). 한국어 독자는
`밥상 찾기` · `자리 요청` · `나눠 먹기` 세 라벨만 보고, 각 단계가 무엇인지
설명하는 문장은 한 줄도 못 봅니다. `MainTab.jsx`의 `main-zig`와
`TablesTab.jsx`의 `how-strip`, 두 화면 다 같습니다.

### 초안 (승인 전)

```
1  혼자보다 같이 먹고 싶은 요리가 올라온 밥상을 찾아보세요.
2  그 자리를 청하세요. 호스트가 어떤 사람인지 읽고 이름을 불러 답합니다 —
   이름이 곧 신청서 전부예요.
3  만나서 같이 먹어요. 밥친구는 돈을 만지지 않습니다. 먹은 만큼 식당에 직접
   내니까 밥상에서 누가 누구에게 빚지는 일이 없어요.
```

3번 뒷문장은 `safetyPromise.js`의 기존 ko를 그대로 씁니다.

### 렌더 방법

`howItWorks.js`의 각 단계에 `ko` 키를 더하고 (`kr`은 짧은 라벨이라 그대로),
두 컴포넌트에서 `say()`에 넘긴 뒤 슬롯 이름을 바꿉니다:

- `MainTab.jsx:393` / `TablesTab.jsx:926`
  `{say(s.en, null, s.es, …)}` → `{say(s.en, s.ko, s.es, …)}`
- 클래스 `main-zig__en` → `main-zig__say`, `how-strip__en` → `how-strip__say`
  (`__en`으로 끝나지 않아야 숨김 규칙에 안 걸립니다)
- `index.css:11406` / `9560`의 선택자도 같이
...
`-kr`/`-en` 쌍이라 한국어 쪽에 같은 내용이 있습니다. 확인된 결손은
`main-zig__en` / `how-strip__en` 둘(같은 데이터, 두 화면)입니다.

나머지 41개를 한 번씩 열어 짝을 확인한 것은 아닙니다. 확인 방법은
**한국어 설정으로 각 화면을 열어 짧아지는 곳을 찾는 것**이지, DOM을 세는 것이
아닙니다 — 짝이 있는지 없는지는 데이터에 있고, 사라진 뒤의 화면에는 없습니다.
...
```bash
npm test                      # 1071 tests — must stay green
