...
## 무엇을 어떻게 확인했나 — "확인했다"와 다릅니다

2026-09-03 착지 시점의 상태입니다. 확인된 것과 확인 방법이 다른 것을 갈라 적습니다.
다음 사람이 "확인 완료"만 보고 넘어가지 않도록.

| 무엇 | 어떻게 | 상태 |
|---|---|---|
| 56개 문자열이 이 문서와 코드에서 같은가 | `scripts/audit-consent-text.mjs`, 한 글자씩, 정규화 없음 | **확인됨** |
| 배포 번들에 새 문자열이 있고 옛 문자열이 없는가 | 배포된 `index-*.js`를 받아 grep | **확인됨** |
| 게이트가 8개 로케일에서 두 문 모두 옳게 렌더되는가 | 실제 컴포넌트를 실제 데이터로 마운트해 DOM을 읽음 | **확인됨** |
| `PURPOSE.rule`·`ifBroken`이 7개 언어로 렌더되는가 | 같은 방식 + ko·en은 **실제 앱의 안전 시트**에서 | **확인됨** |
| **게이트 네 줄과 버튼이 실제 앱 화면에서** | — | **미완** |
| 동의를 눌러도 기록되지 않고, 나갔다 오면 게이트가 다시 서는가 | 실제 컴포넌트를 마운트·언마운트하며 흐름을 구동 | **확인됨** (2026-09-03) |
| **밥상이 실제로 만들어졌을 때 기록되는가** | — | **미완** — 진짜 밥상을 만들어야 확인됩니다 |

**미완인 이유**를 적어둡니다. 동의 게이트는 `TableCreate`와 `TableDetail` 양쪽
모두 **가입 뒤에** 있습니다(`상을 차리려면 가입이 필요해요` /
`자리를 요청하려면 가입이 필요해요`). 확인하려면 계정이 필요하고, 계정을 만드는
것은 이 저장소의 규칙이 금지합니다. 그래서 컴포넌트를 따로 마운트했습니다 —
컴포넌트·데이터·`say()`는 실물이지만 부모가 다릅니다.

**`PURPOSE.rule`과 `ifBroken`은 계정 없이 볼 수 있습니다.** 안전 시트
(도움이 필요하면)는 누구에게나 열립니다. 그 둘은 실제 화면에서 확인된 것이 맞습니다.

### 다음 기회는 `PURPOSE.version`을 올릴 때입니다

버전이 올라가면 이미 동의한 사람에게 게이트가 **다시 뜹니다.** 그것이 실제 앱
화면에서 네 줄과 버튼을 볼 유일한 기회입니다 — 평소에는 처음 한 번뿐이고,
그 한 번은 이미 지나갔습니다.

**그때 확인할 것:** 네 줄이 읽는 사람의 언어인가 · 버튼이
`동의하고 계속 — 상 차리기` / `동의하고 계속 — 자리 요청`인가(영어 조각이
섞이지 않았는가) · 두 문에서 버튼 문구가 서로 다른가 · 360px에서 줄이 깨지지 않는가.
...
## 언제 기록되나 — 2026-09-03에 바뀌었습니다

**동의 버튼은 그 화면에서 폼을 열 뿐이고, 기록은 행동이 끝났을 때 만들어집니다.**

- 상 차리기: 밥상이 실제로 만들어졌을 때
- 자리 요청: 신청이 실제로 접수됐을 때

중간에 나가면 아무것도 기록되지 않고, 다시 들어오면 게이트가 다시 뜹니다.
`agreedHere`는 컴포넌트 상태라 화면이 언마운트되면 같이 사라집니다.

**기록되는 시각은 버튼을 누른 순간입니다**, 완료 순간이 아니라. 완료 시각을
쓰면 아무도 아무것도 읽지 않은 순간이 동의 시각으로 남습니다.

그전에는 버튼이 곧 기록이었습니다. 그 동작을 "동의는 규칙에 대한 동의이지
행동을 끝내겠다는 약속이 아니다"라는 설계 판단으로 방어했고, 팀이 기각했습니다.
**애초에 이쪽에서 정할 일이 아니었습니다.**

### 경계 사례 하나 — `rules_version = 2`인 프로필 둘

둘 다 그대로 둡니다. 다만 그중 하나는 **버튼만 누르고 밥상을 만들지 않은**
경우입니다 — 옛 규칙에서는 그것으로 기록됐고, 새 규칙이었다면 기록되지
않았을 것입니다. 옛 규칙 아래에서는 유효한 기록이므로 손대지 않습니다.

### 각주가 가리키던 곳이 여섯 언어에 없었습니다

각주는 "도움이 필요하면에서 다시 읽으실 수 있어요"라고 일곱 언어 모두에
적혀 있었습니다. 그런데 그 라벨은 `journal-tool__kr` 스팬이고
`index.css:11739`가 `__kr`로 끝나는 클래스를 한국어 밖 여섯 로케일에서
전부 숨깁니다. 배포 스타일시트로 재서 확인했습니다:

```
both / ko      display=block   "도움이 필요하면 / 112, 119, …"
en es fr ar zh ja  display=none    "112, 119, …"
```

여섯 명 중 여섯이 **자기 화면에 없는 글자를 찾으라는 안내**를 읽고 있었습니다.
이제 일곱 언어 모두 **여권 탭**을 가리킵니다 — `TabBar.jsx:76`이 언어별로
번역합니다(Passport · Pasaporte · Passeport · الجواز · 护照 · パスポート).
한국어도 절 이름이 아니라 탭 이름을 씁니다: 일곱이 같은 이정표를 가리키는
것이 한 언어의 걸음 하나보다 낫습니다.
