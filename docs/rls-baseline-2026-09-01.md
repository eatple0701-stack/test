...
# 2026-09-03a — 동의는 뒤로 갈 수 없다 (적용 완료)

`supabase/migrations/2026-09-03a-consent-cannot-go-backwards.sql`

## 왜 이 절이 이 문서에서 제일 값어치 있나

**이력 테이블을 만든 지 하루 만에, 그것 없이는 아무도 못 봤을 사건을 잡았습니다.**
그리고 이 절은 그 사건 자체보다 **왜 스칼라 두 칸으로는 부족한지**의 실물
증거로 남깁니다.

## 무엇이 보였나

`PURPOSE.version`을 1→2로 올린 지 여섯 시간, 배포한 지 40분 뒤:
...
백필이 v1을 열 줄 썼으니 **새 줄이 둘**이고, `profiles`에는 v2가 **하나도
없습니다.** 어떤 프로필이 1 → 2 → 다시 1로 갔습니다.

**`profiles`만 봤다면 이 사건은 존재하지 않습니다.** v2가 없으니
"동의를 안 눌렀나 보다"로 끝납니다. 실제로 그것이 첫 가설이었고,
숫자가 그 가설을 반증했습니다.

## 되돌린 줄의 `agreed_at`이 원인을 두 번 좁혔다

```
version  agreed_at                 recorded_at
2        10:11:30.25               10:11:30.729
1        10:18:24.808              10:18:25.352      ← 6분 54초 뒤
```

되돌린 줄의 `agreed_at`이 **옛 날짜가 아니라 그 순간 만들어진 시각**이고,
`recorded_at`보다 0.5초 앞섭니다.

이 사실 하나가 두 가설을 죽였습니다.

**하나.** 낡은 탭이 메모리의 옛 프로필을 통째로 보냈다는 가설 — 그랬다면
`agreed_at`은 8월 날짜여야 합니다. 새 시각이 찍혔다는 것은
`rulesAgreement()`가 **그 순간 실행됐다**는 뜻이고, 그 함수는 동의 버튼
`onClick` 한 곳에서만 불립니다.

**둘.** 마이크로초 절삭으로 중복 행이 생겼다는 가설 — 11번째 줄은 중복이
아니라 실제로 일어난 두 번째 사건이었습니다. 유니크 제약은 제 일을 했습니다.

남은 결론은 하나입니다. **`PURPOSE.version`이 1인 클라이언트에서 동의 버튼이
눌렸다.** 배포 전부터 열려 있던 탭입니다. `agreedToRules`가
`agreed === currentVersion`이라, 그 탭의 게이트는 `agreed === 1`을 물었고
프로필은 2였고, **이미 더 최신 버전에 동의한 사람에게 옛 게이트를 보여준 뒤
그 동의를 후퇴시켰습니다.**

## 스칼라 두 칸이 못 하는 것

| | `profiles` 두 칸 | `rules_consents` |
|---|---|---|
| 지금 무엇에 동의했나 | ○ | ○ |
| 전에 무엇에 동의했나 | ✗ 덮어써짐 | ○ |
| 순서 | ✗ | ○ `recorded_at` |
| 언제 합의됐고 언제 기록됐나 | 한 칸뿐 | 두 칸 — 이 사건은 **그 차이**로 풀렸습니다 |

`agreed_at`과 `recorded_at`을 **따로** 둔 것이 결정적이었습니다. 하나였다면
"새 시각인가 옛 시각인가"를 물을 수 없고, 낡은 프로필과 낡은 번들을
가를 수 없었습니다.

## 세 겹으로 고쳤다

1. **`agreedToRules`를 `>=`로.** 근본 원인. 옛 번들이 더 높은 버전에 동의한
   사람에게 게이트를 못 띄웁니다. 옛 테스트 하나가 "미래 버전은 조작된
   저장소에서만 온다"고 단언하고 있었는데, 그 전제가 이 사건으로 깨졌습니다 —
   배포 뒤 새로고침 안 한 탭에서도 옵니다. 지우지 않고 뒤집으면서 왜 틀렸는지
   적었습니다.
2. **DB 가드.** `rules_version`이 내려가거나 null이 되려 하면 조용히 옛 값을
   유지합니다. 버전과 시각을 **함께** 잡습니다 — 번호만 지키고 시각이 덮이면
   "뒤로 밀린 그 순간에 동의했다"는 기록이 됩니다. 예외는 던지지 않습니다.
3. **클라이언트가 일반 저장에 동의를 안 싣는다.** 이름 한 글자를 고쳐도
   동의가 함께 날아가던 구조였습니다. 마이크로초 왕복 절삭도 같이 사라졌습니다.

## 적용 후 확인된 값

트랜잭션 안에서 아홉 칸, 커밋 뒤 트랜잭션 밖에서 여섯 칸.

```
guard_before_update  true    profiles_at_v2  1     history_rows   12
guard_path_pinned    true    profiles_at_v1  9     history_at_v2  1
```

되돌려졌던 프로필이 **이력의 원래 시각으로** 복구됐고, 복구가 이력을 늘리지
않았습니다(`on conflict do nothing`) — 새로 합의된 것이 없으니 새 기록도
없습니다.

## 검증 장치가 두 번 틀렸다 — 이쪽도 기록해 둡니다

**첫 붙여넣기에서 아홉 칸 중 셋이 NULL이었습니다.** 검증부를 data-modifying
CTE 사슬로 짰는데, 한 문장 안의 서브문장들은 같은 스냅숏을 보고 실행 순서가
정의되지 않습니다. 같은 행을 노린 네 개의 UPDATE 중 **하나만 적용되고 셋은
버려졌습니다.** PGlite에서 재현했습니다 — 이름이 바뀌지 않았고 다섯 값 중
셋이 null이었습니다. **NULL은 false가 아니라 아무것도 재지 못한 것이고,
같은 장치에서 나온 나머지 값도 그만큼 못 믿습니다.**

**PGlite 테스트 열 개는 그때 전부 초록이었습니다.** 그 테스트들은
마이그레이션의 **적용부만** 실행하고 검증은 따로 짠 순차 쿼리로 했습니다.
가드는 검증했지만 **probe는 한 번도 실행된 적이 없었습니다.**

그래서 `migrationProbe.test.mjs`가 생겼습니다 — 마이그레이션 파일을
**통째로, 한 트랜잭션으로** 돌리고 아홉 칸이 (a) NULL이 아니고 (b) 문서의
값과 같은지 봅니다. **사람에게 숫자를 읽으라고 하는 마이그레이션은 전부 여기
들어와야 합니다.**

두 번째 실수는 `- 12`였습니다. 프로덕션의 그 시각 이력 개수를 상수로 박아서
다른 DB에서는 뜻이 없고, 붙여넣기 전에 누가 하나라도 동의하면 프로덕션에서도
틀립니다. 이제 파일이 **적용 전 개수를 스스로 세어** 차이를 냅니다.
