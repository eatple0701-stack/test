// Korean food culture content, shared by category (restaurant.category).
// Written for curious first-time visitors: one hook that sparks curiosity,
// then short practical dining tips. Individual restaurants can override by
// adding `didYouKnow` / `diningTips` fields to their own data entry.

export const cultureByCategory = {
  temple: {
    didYouKnowEs: "La cocina de templo coreana no usa ajo, cebolla, cebollino, puerro ni cebolleta — las llamadas cinco verduras picantes — porque los monjes consideran que agitan las emociones. Todo el sabor profundo que notas aquí viene de la fermentación y de la paciencia.",
    diningTipsEs: [
          "Come como un monje: terminar lo que hay en el cuenco es el sentido de la práctica — el 발우공양 no deja ni un grano de arroz.",
          "Prueba las guarniciones una a una. Cada banchan está sazonado para comerse con arroz, no por separado.",
          "Aquí se come en silencio y sin prisa: la gente lo trata como meditación, no como un almuerzo."
    ],
    whyLocalsLoveItEs: "Los habituales vuelven tanto por el silencio como por la comida: una mesa de templo es de los pocos sitios de Seúl donde nadie mira el móvil.",
    conversationTipsEs: [
          "Pregunta al personal qué significa 발우공양 — casi siempre les gusta explicar el ritual del cuenco vacío.",
          "Baja la voz: el comedor de un templo se trata como una extensión de la sala de meditación."
    ],
    passportMissionEs: {
          "title": "El cuenco vacío",
          "detail": "Termina hasta el último grano de arroz, como manda el 발우공양. No dejar nada no es buena educación: es la práctica entera."
    },
    culturalMeaningEs: "La comida de templo encarna una idea budista de comer con atención: nada se desperdicia, nada es exceso, todo es intencionado.",
    whenKoreansEatThisEs: "Durante un templestay, en una fiesta budista, o cuando alguien quiere un respiro tranquilo de la comida de ciudad.",
    didYouKnowFr: "La cuisine de temple coréenne bannit l'ail, l'oignon, la ciboulette, le poireau et l'oignon vert — les « cinq légumes piquants » — parce que les moines estiment qu'ils agitent les émotions. Toute la profondeur que vous goûtez ici vient de la fermentation et de la patience.",
    diningTipsFr: [
          "Mangez comme un moine : finir son bol est le sens même de la pratique — le 발우공양 ne laisse pas un grain de riz.",
          "Goûtez les petits plats un par un. Chaque banchan est assaisonné pour être mangé avec du riz, pas seul.",
          "Ici on mange en silence et sans hâte : les gens d'ici y voient une méditation, pas un déjeuner.",
    ],
    whyLocalsLoveItFr: "Les habitués reviennent autant pour le calme que pour la nourriture : une table de temple est l'un des rares endroits de Séoul où personne ne regarde son téléphone.",
    conversationTipsFr: [
          "Demandez au personnel ce que veut dire 발우공양 — la plupart expliquent volontiers le rituel du bol vide.",
          "Baissez la voix : la salle à manger d'un temple se traite comme une extension de la salle de méditation.",
    ],
    passportMissionFr: { title: "Le bol vide", detail: "Finissez jusqu'au dernier grain de riz, comme le veut le 발우공양. Ne rien laisser n'est pas de la politesse : c'est toute la pratique." },
    culturalMeaningFr: "La cuisine de temple porte une idée bouddhiste du manger attentif : rien de gaspillé, rien de complaisant, tout est intentionnel.",
    whenKoreansEatThisFr: "Pendant un templestay, lors d'une fête bouddhiste, ou quand on veut souffler loin de la nourriture de ville.",
    didYouKnowAr: "يحرّم مطبخ المعبد الكوري الثوم والبصل والثوم المعمّر والكرّاث والبصل الأخضر — ما يسمّى الخضار الخمس الحرّيفة — لأن الرهبان يرون أنها تهيّج المشاعر. وكل عمق تذوقه هنا يأتي من التخمير والصبر.",
    diningTipsAr: [
          "كُل كما يأكل الراهب: إفراغ قصعتك هو معنى الممارسة نفسه — فالـ발우공양 لا يترك حبّة أرز.",
          "ذُق الأطباق الصغيرة واحدًا واحدًا. كل بانتشان مُتبّل ليُؤكل مع الأرز لا وحده.",
          "الطعام هنا صامت وبلا عجلة: يراه أهل المكان تأمّلًا لا غداءً.",
    ],
    whyLocalsLoveItAr: "يعود المداومون للهدوء بقدر ما يعودون للطعام: مائدة المعبد من الأماكن القليلة في سول التي لا ينظر فيها أحد إلى هاتفه.",
    conversationTipsAr: [
          "اسأل العاملين ما معنى 발우공양 — معظمهم يسرّهم شرح طقس القصعة الفارغة.",
          "اخفض صوتك: تُعامَل قاعة طعام المعبد امتدادًا لقاعة التأمّل.",
    ],
    passportMissionAr: { title: "القصعة الفارغة", detail: "أنهِ كل حبّة أرز، كما يقصد الـ발우공양. ألّا تترك شيئًا ليس أدبًا: هو الممارسة كلّها." },
    culturalMeaningAr: "يحمل طعام المعبد فكرة بوذية عن الأكل بانتباه: لا شيء يُهدر، ولا شيء فيه إفراط، وكل شيء مقصود.",
    whenKoreansEatThisAr: "أثناء إقامة في معبد، أو في عيد بوذي، أو حين يريد المرء راحة هادئة من طعام المدينة.",
    didYouKnowZh: "韩国的寺院饮食不用蒜、洋葱、韭菜、葱和野蒜——所谓\"五辛\"——因为僧人认为它们会搅动强烈的情绪。你在这里尝到的所有深味，都来自发酵和慢慢等。",
    diningTipsZh: [
          "像僧人那样吃：把碗里的东西吃完就是这件事的意思——발우공양连一粒米都不剩。",
          "小菜一样一样地尝。每一道 banchan 的调味都是配着米饭吃的，不是单独吃的。",
          "这里的饭安静而不赶：本地人把它当作打坐，而不是午餐。",
    ],
    whyLocalsLoveItZh: "常客回来，为的是那份安静，不比为食物少——寺院的饭桌是首尔少有的、没人在看手机的地方。",
    conversationTipsZh: [
          "问问店里的人 발우공양 是什么意思——多数人都乐意讲讲空碗的这个仪式。",
          "把声音放低：寺院的斋堂被当作禅堂的延伸。",
    ],
    passportMissionZh: { title: "空碗", detail: "把每一粒米都吃完，像발우공양本来的意思那样。什么都不剩下不是礼貌，是全部的修行。" },
    culturalMeaningZh: "寺院的饭承载着一种佛教式的、带着觉察的吃：不浪费，不纵容，每一样都有意思。",
    whenKoreansEatThisZh: "在寺院寄宿时，佛教节日里，或者谁想从城市的饭里安静地缓一缓的时候。",
    didYouKnowKo: "한국의 사찰음식은 마늘, 양파, 부추, 파, 달래 — 이른바 오신채 — 를 쓰지 않습니다. 스님들이 이것들이 강한 감정을 일으킨다고 보기 때문이에요. 여기서 느끼는 깊은 맛은 전부 발효와 오래 기다린 시간에서 옵니다.",
    diningTipsKo: [
          "스님처럼 드세요. 그릇을 비우는 것이 핵심입니다 — 발우공양은 밥알 하나 남기지 않는 수행이에요.",
          "반찬은 하나씩 천천히 맛보세요. 각각 밥과 같이 먹도록 간을 맞춘 것이지, 따로 먹으라고 만든 게 아닙니다.",
          "여기서의 식사는 조용하고 서두르지 않습니다. 현지 사람들은 점심이라기보다 명상으로 여겨요."
    ],
    whyLocalsLoveItKo: "단골들은 음식만큼이나 그 고요함 때문에 다시 옵니다. 서울에서 아무도 휴대폰을 들여다보지 않는 몇 안 되는 밥상이에요.",
    conversationTipsKo: [
          "직원분께 발우공양이 무슨 뜻인지 물어보세요. 대부분 그 빈 그릇의 의미를 기꺼이 설명해 줍니다.",
          "목소리는 낮게. 절의 공양간은 선방의 연장으로 여겨집니다."
    ],
    passportMissionKo: {
          "title": "빈 그릇",
          "detail": "발우공양이 뜻하는 대로 밥알 하나까지 비워 보세요. 아무것도 남기지 않는 것이 예의가 아니라 수행 그 자체입니다."
    },
    culturalMeaningKo: "사찰음식은 알아차리며 먹는다는 불교의 태도를 담고 있습니다. 버리는 것 없이, 탐하는 것 없이, 모든 것이 뜻을 가지고요.",
    whenKoreansEatThisKo: "템플스테이 중에, 불교 명절에, 또는 도시의 음식에서 잠시 벗어나고 싶을 때.",
    didYouKnow:
      "Korean temple cuisine bans garlic, onions, chives, leeks and green onions — the 'five pungent vegetables' — because monks believe they stir up strong emotions. Every deep flavor you taste here comes from fermentation and slow patience instead.",
    diningTips: [
      'Eat like a monk: finishing everything in your bowl is the point — the practice called 발우공양 (balwoo gongyang) leaves not a single grain of rice behind.',
      'Taste the small dishes one by one. Each banchan is seasoned to be eaten with rice, not on its own.',
      'Meals here are quiet and unhurried — locals treat them as meditation, not just lunch.',
    ],
    whyLocalsLoveIt:
      "Regulars come back for the quiet as much as the food — a temple meal is one of the few places in Seoul where no one is checking their phone.",
    usefulKorean: [
      { ko: '잘 먹겠습니다', ro: 'jal meokkesseumnida', en: 'I will eat well (said before the meal)', ko_gloss: '식사 전에 하는 인사', es: 'Comeré bien (se dice antes de comer)', fr: "Je vais bien manger (se dit avant le repas)", ar: "سآكل جيدًا (تُقال قبل الطعام)", zh: "我要好好吃了（饭前说的）" },
      { ko: '공양', ro: 'gongyang', en: 'a temple meal offering', ko_gloss: "절에서 올리는 공양", es: "la ofrenda de una comida de templo", fr: "l'offrande d'un repas de temple", ar: "قربان وجبة في المعبد", zh: "寺院供养的一餐" },
      { ko: '맛있어요', ro: 'masisseoyo', en: "It's delicious", ko_gloss: "맛있다는 말", es: "Está delicioso", fr: "C'est délicieux", ar: "إنه لذيذ", zh: "很好吃" },
    ],
    conversationTips: [
      'Ask the staff what 발우공양 means — most are happy to explain the empty-bowl ritual to curious travelers.',
      'Keep your voice low; a temple dining room is treated as an extension of the meditation hall.',
    ],
    passportMission: { title: 'The Empty Bowl', detail: 'Finish every grain of rice, the way 발우공양 intends. Leaving nothing behind is the whole practice, not just good manners.' },
    culturalMeaning: 'Temple food represents a Buddhist philosophy of eating with awareness — nothing wasted, nothing indulgent, everything intentional.',
    whenKoreansEatThis: 'During a templestay, on a Buddhist holiday, or whenever someone wants a quiet reset from city food.',
  },
  'korean-chinese': {
    didYouKnowEs: "El jajangmyeon nació en el barrio chino de Incheon hacia 1905, inventado por estibadores chinos lejos de casa. Hoy los coreanos comen millones de cuencos al día, y sigue siendo la comida de mudanza que se pide al suelo del piso nuevo.",
    diningTipsEs: [
          "Mezcla bien la salsa de judía negra con los fideos antes del primer bocado, y come rápido, antes de que se hinchen.",
          "El rábano amarillo encurtido (danmuji) está para cortar la grasa. Alterna bocados.",
          "Sorber es perfectamente educado aquí: enfría los fideos y señala que te están gustando."
    ],
    whyLocalsLoveItEs: "Es lo que come Seúl el día de la mudanza y también las noches solitarias: el jajangmyeon es consuelo antes que cocina.",
    conversationTipsEs: [
          "Pregunta al personal en qué local comían de pequeños — todo el mundo en Corea tiene su jjajangmyeon de la infancia.",
          "Saca el tema de 짜장 contra 짬뽕: esa rivalidad rompe el hielo sin fallo."
    ],
    passportMissionEs: {
          "title": "Pedir como un local",
          "detail": "Pide un jjajangmyeon y un jjamppong para la mesa e intercambiadlos a mitad: la solución coreana estándar a una elección imposible."
    },
    culturalMeaningEs: "El jajangmyeon guarda la memoria de la comunidad china de Corea: un plato que se volvió del todo coreano conservando su nombre extranjero.",
    whenKoreansEatThisEs: "El día de la mudanza, después de trabajar hasta tarde, o cualquier día en que nadie quiere decidir.",
    didYouKnowFr: "Le jajangmyeon est né dans le quartier chinois d'Incheon vers 1905, inventé par des dockers chinois loin de chez eux. Les Coréens en mangent aujourd'hui des millions de bols par jour, et c'est toujours le repas du déménagement, livré à même le sol du nouvel appartement.",
    diningTipsFr: [
          "Mélangez bien la sauce de haricots noirs aux nouilles avant la première bouchée — et mangez vite, avant que les nouilles ne gonflent.",
          "Le radis jaune mariné (danmuji) est là pour couper le gras. Alternez les bouchées.",
          "Aspirer ses nouilles est parfaitement poli ici : cela les refroidit et montre que vous vous régalez.",
    ],
    whyLocalsLoveItFr: "C'est ce que Séoul mange le jour du déménagement comme les soirs de solitude : le jajangmyeon est un réconfort avant d'être une cuisine.",
    conversationTipsFr: [
          "Demandez dans quelle boutique de nouilles le personnel a grandi — tout le monde en Corée a son jjajangmyeon d'enfance.",
          "Lancez 짜장 contre 짬뽕 (haricots noirs contre fruits de mer épicés) : cette rivalité est un brise-glace fiable.",
    ],
    passportMissionFr: { title: "Commandez comme un habitué", detail: "Prenez un jjajangmyeon et un jjamppong pour la table et échangez à mi-chemin — la solution coréenne standard à un choix impossible." },
    culturalMeaningFr: "Le jajangmyeon porte la mémoire de la communauté immigrée chinoise de Corée : un plat devenu entièrement coréen tout en gardant son nom étranger.",
    whenKoreansEatThisFr: "Le jour du déménagement, le jour des bulletins, ou n'importe quel soir de semaine où personne n'a envie de cuisiner.",
    didYouKnowAr: "وُلد الجاجانغميون في الحي الصيني بإنتشون نحو عام 1905، اخترعه عمّال مرفأ صينيون بعيدون عن ديارهم. واليوم يأكل الكوريون ملايين الصحون يوميًا، وهو ما زال وجبة يوم الانتقال، تُوصَّل إلى أرض الشقة الجديدة.",
    diningTipsAr: [
          "اخلط صلصة الفول الأسود بالمعكرونة جيدًا قبل أول لقمة — وكُل بسرعة قبل أن تنتفخ المعكرونة.",
          "الفجل الأصفر المخلّل (دانموجي) على الجانب موجود ليقطع الدسم. ناوب بين اللقمات.",
          "ارتشاف المعكرونة مهذّب تمامًا هنا: يبرّدها ويقول إنك مستمتع بها.",
    ],
    whyLocalsLoveItAr: "هو ما تأكله سول يوم الانتقال وفي الليالي الموحشة سواء: الجاجانغميون سلوى قبل أن يكون مطبخًا.",
    conversationTipsAr: [
          "اسأل العاملين في أي محل معكرونة كبروا وهم يأكلون — لكل واحد في كوريا جاجانغميون طفولة.",
          "اذكر 짜장 مقابل 짬뽕 (الفول الأسود مقابل البحريات الحارّة): هذه المنافسة كاسر جليد لا يخيب.",
    ],
    passportMissionAr: { title: "اطلب كأهل المكان", detail: "خذ جاجانغميون واحدًا وجامبونغ واحدًا للطاولة وتبادلوهما في المنتصف — الحلّ الكوري المعتاد لاختيار لا يُحسم." },
    culturalMeaningAr: "يحمل الجاجانغميون ذاكرة الجالية الصينية المهاجرة في كوريا: طبق صار كوريًّا تمامًا وأبقى اسمه الأجنبي.",
    whenKoreansEatThisAr: "يوم الانتقال، أو يوم صدور شهادات المدرسة، أو أي ليلة أسبوع لا يريد أحد فيها أن يطبخ.",
    didYouKnowZh: "炸酱面约在1905年诞生于仁川的中华街，由远离家乡的中国码头工人发明。今天韩国人一天要吃掉几百万碗，而它仍然是搬家那天的饭——送到新公寓还空着的地板上。",
    diningTipsZh: [
          "第一口之前把黑豆酱和面彻底拌开——然后快点吃，别等面泡涨。",
          "旁边那碟黄色腌萝卜（단무지）是用来解腻的。换着吃。",
          "出声地吸面在这里完全不失礼——既能把面吹凉，也表示你吃得香。",
    ],
    whyLocalsLoveItZh: "这是首尔在搬家那天和在孤单的夜里都会吃的东西：炸酱面首先是安慰，其次才是菜。",
    conversationTipsZh: [
          "问问店员是在哪家面馆长大的——每个韩国人都有一家童年的炸酱面店。",
          "提一句 짜장 和 짬뽕（黑豆酱面和辣海鲜面）——这场对立是很可靠的破冰话题。",
    ],
    passportMissionZh: { title: "像本地人那样点", detail: "给这桌点一份炸酱面和一份炒码面，吃到一半互换——面对一个选不出来的选择，韩国人的标准解法。" },
    culturalMeaningZh: "炸酱面承载着韩国华人社群的记忆：一道完全变成韩国菜、却留着外来名字的菜。",
    whenKoreansEatThisZh: "搬家那天，成绩单发下来那天，或者任何一个谁都不想做饭的工作日晚上。",
    didYouKnowKo: "짜장면은 1905년 무렵 인천 차이나타운에서, 고향을 멀리 떠나온 중국인 부두 노동자들의 손에서 태어났습니다. 지금 한국 사람들은 하루에 수백만 그릇을 먹고, 여전히 이사하는 날 새 집 바닥에 배달시켜 먹는 음식이에요.",
    diningTipsKo: [
          "첫 젓가락 전에 춘장을 면에 골고루 비비세요. 그리고 면이 붇기 전에 빨리 드세요.",
          "옆에 나오는 단무지는 느끼함을 잘라 주는 역할입니다. 번갈아 드세요.",
          "후루룩 소리를 내도 전혀 무례하지 않습니다. 면을 식히고, 맛있게 먹고 있다는 신호이기도 해요."
    ],
    whyLocalsLoveItKo: "이사하는 날에도, 혼자인 밤에도 서울이 먹는 음식입니다. 짜장면은 요리이기 전에 위로예요.",
    conversationTipsKo: [
          "직원분께 어릴 때 어느 중국집에서 드셨는지 물어보세요. 한국 사람은 누구나 어린 시절의 짜장면집이 있습니다.",
          "짜장이냐 짬뽕이냐를 꺼내 보세요. 이 논쟁은 확실한 대화의 물꼬입니다."
    ],
    passportMissionKo: {
          "title": "현지인처럼 주문하기",
          "detail": "짜장면 하나와 짬뽕 하나를 시켜 놓고 중간에 바꿔 드세요. 결론이 안 나는 선택에 대한 한국식 해법입니다."
    },
    culturalMeaningKo: "짜장면은 한국 화교 사회의 기억을 품고 있습니다. 외국 이름을 그대로 단 채 완전히 한국 음식이 된 요리예요.",
    whenKoreansEatThisKo: "이사하는 날, 야근 뒤, 혹은 아무 결정도 하기 싫은 날.",
    didYouKnow:
      "Jajangmyeon was born in Incheon's Chinatown around 1905, invented by Chinese dockworkers far from home. Today Koreans eat millions of bowls a day — and it is still the traditional 'moving day' meal, delivered to your new apartment floor.",
    diningTips: [
      'Mix the black bean sauce into the noodles thoroughly before your first bite — and eat fast, before the noodles swell.',
      'The yellow pickled radish (danmuji) on the side is there to cut the richness. Alternate bites.',
      "Slurping is perfectly polite here — it cools the noodles and signals you're enjoying them.",
    ],
    whyLocalsLoveIt:
      "It's the food Seoul eats on moving day and lonely nights alike — jajangmyeon is comfort food first, cuisine second.",
    usefulKorean: [
      { ko: '곱빼기', ro: 'gopbaegi', en: 'double portion', ko_gloss: "곱빼기 = 두 배", es: "ración doble", fr: "portion double", ar: "حصة مضاعفة", zh: "加大份" },
      { ko: '단무지 더 주세요', ro: 'danmuji deo juseyo', en: 'More pickled radish, please', ko_gloss: "단무지를 더 달라는 말", es: "Más rábano encurtido, por favor", fr: "Encore du radis mariné, s'il vous plaît", ar: "المزيد من الفجل المخلّل، من فضلك", zh: "请再来点腌萝卜" },
      { ko: '짜장면 하나요', ro: 'jjajangmyeon hanayo', en: 'One jajangmyeon, please', ko_gloss: "짜장면 하나 주문", es: "Un jajangmyeon, por favor", fr: "Un jajangmyeon, s'il vous plaît", ar: "جاجانغميون واحد، من فضلك", zh: "一份炸酱面，谢谢" },
    ],
    conversationTips: [
      'Ask which noodle shop the staff grew up eating at — everyone in Korea has a childhood jjajangmyeon spot.',
      'Mention 짜장 vs 짬뽕 (black bean vs spicy seafood noodles) — the rivalry is a reliable icebreaker.',
    ],
    passportMission: { title: 'Order Like a Local', detail: 'Get one jjajangmyeon and one jjamppong for the table and swap halfway — the standard Korean solution to an unwinnable choice.' },
    culturalMeaning: "Jajangmyeon carries the memory of Korea's Chinese immigrant community — a dish that became fully Korean while keeping its foreign name.",
    whenKoreansEatThis: 'Moving day, the day report cards come out, or any weeknight nobody wants to cook.',
  },
  'vegan-dining': {
    didYouKnowEs: "La escena vegetal de Corea se apoya en una idea muy antigua: 나물, el arte de aliñar verduras silvestres. Mucho antes de que existiera la palabra \"vegano\", una mesa coreana en condiciones ya se construía sobre decenas de platos de verdura.",
    diningTipsEs: [
          "Prueba los platos de carne de soja aunque te den reparo: las cocinas coreanas llevan décadas afinando esa textura en la cocina de templo.",
          "Las guarniciones (banchan) suelen rellenarse. Pedir más es un cumplido a la cocina, no un desliz."
    ],
    whyLocalsLoveItEs: "Las cocinas vegetales de Seúl juntan a novicios, atletas y escépticos: todos salen discutiendo si sabía o no a carne.",
    conversationTipsEs: [
          "Pregunta de qué está hecha la carne de soja: casi todas las cocinas veganas de Seúl están orgullosas de la técnica y les encanta explicarla.",
          "Los veganos coreanos siguen siendo una comunidad pequeña y unida; el personal suele conocer todos los demás sitios vegetales de la ciudad."
    ],
    passportMissionEs: {
          "title": "Pregunta en la cocina",
          "detail": "Pregunta de qué está hecha la carne de soja. Las cocinas vegetales coreanas están orgullosas de la técnica y te la explicarán encantadas."
    },
    culturalMeaningEs: "Comer vegetal en Corea se remonta siglos más atrás que la palabra \"vegano\": las cocinas de hoy están recuperando una mesa mucho más antigua y centrada en la verdura.",
    whenKoreansEatThisEs: "Cada vez, cualquier día: la escena vegana de Seúl ha pasado de nicho a categoría propia en la última década.",
    didYouKnowFr: "La scène végétale coréenne repose sur une idée très ancienne : le 나물 (namul), l'art d'assaisonner les herbes sauvages. Bien avant que « végane » soit un mot, une vraie table coréenne se construisait déjà autour de dizaines de plats de légumes.",
    diningTipsFr: [
          "Goûtez les plats de soja même si vous êtes sceptique — les cuisines coréennes en ont affiné la texture par des décennies de cuisine de temple.",
          "Les accompagnements (banchan) se resservent en général. En redemander est un compliment à la cuisine, pas une faute.",
    ],
    whyLocalsLoveItFr: "Les cuisines végétales de Séoul attirent un public mêlé de novices, de sportifs et de sceptiques : tout le monde ressort en discutant de savoir si cela avait le goût de la viande.",
    conversationTipsFr: [
          "Demandez de quoi est faite la viande de soja — la plupart des cuisines véganes de Séoul sont fières de la technique et adorent l'expliquer.",
          "Les véganes coréens forment encore une petite communauté soudée : le personnel connaît souvent toutes les autres adresses de la ville.",
    ],
    passportMissionFr: { title: "Interrogez la cuisine", detail: "Demandez de quoi est faite la viande de soja. Les cuisines végétales coréennes sont fières de leur technique et vous l'expliqueront volontiers." },
    culturalMeaningFr: "Manger végétal en Corée remonte des siècles avant le mot « végane » : les cuisines d'aujourd'hui font revivre une table bien plus ancienne, centrée sur le légume.",
    whenKoreansEatThisFr: "De plus en plus, n'importe quel jour : la scène végane de Séoul est passée en dix ans d'une niche à une catégorie à part entière.",
    didYouKnowAr: "يقوم المشهد النباتي الكوري على فكرة قديمة جدًّا: الـ나물 (نامول)، فنّ تتبيل الأعشاب البرّية. وقبل أن تُوجد كلمة «نباتي صرف» بزمن طويل كانت المائدة الكورية اللائقة مبنيّة أصلًا على عشرات أطباق الخضار.",
    diningTipsAr: [
          "ذُق أطباق لحم الصويا حتى لو كنت متشكّكًا — صقلت المطابخ الكورية قوامها عبر عقود من طبخ المعابد.",
          "الأطباق الجانبية (بانتشان) تُعاد تعبئتها عادةً. وطلب المزيد مجاملة للمطبخ لا زلّة.",
    ],
    whyLocalsLoveItAr: "تجتذب المطابخ النباتية في سول خليطًا من متدرّبي الرهبنة والرياضيين والمتشكّكين: ويخرج الجميع يتجادلون هل كان طعمه كاللحم.",
    conversationTipsAr: [
          "اسأل ممّ يُصنع لحم الصويا — معظم المطابخ النباتية في سول فخورة بالتقنية وتحبّ شرحها.",
          "ما زال النباتيون الكوريون جماعة صغيرة متلاحمة: والعاملون غالبًا يعرفون كل مكان نباتي آخر في المدينة.",
    ],
    passportMissionAr: { title: "اسأل المطبخ", detail: "اسأل ممّ يُصنع لحم الصويا. المطابخ النباتية الكورية فخورة بتقنيتها وستشرحها لك بسرور." },
    culturalMeaningAr: "يعود الأكل النباتي في كوريا إلى قرون قبل كلمة «نباتي صرف»: ومطابخ اليوم تحيي مائدة أقدم بكثير قائمة على الخضار.",
    whenKoreansEatThisAr: "أي يوم، على نحو متزايد: انتقل المشهد النباتي في سول خلال عقد من زاوية ضيّقة إلى صنف قائم بذاته.",
    didYouKnowZh: "韩国的植物性饮食建立在一个很老的想法上：나물（namul），给野菜调味的手艺。在\"纯素\"这个词存在很久之前，一张像样的韩国餐桌上就已经摆着几十道蔬菜。",
    diningTipsZh: [
          "就算你怀疑，也尝一尝豆制的\"肉\"——韩国厨房用几十年的寺院烹饪把那个口感磨出来了。",
          "小菜（banchan）通常可以续。再要一份是对厨房的恭维，不是失礼。",
    ],
    whyLocalsLoveItZh: "首尔的植物性厨房里坐着混杂的人：见习僧人、运动员和怀疑论者——所有人离开时都在争论它到底像不像肉。",
    conversationTipsZh: [
          "问问豆制的\"肉\"是用什么做的——首尔多数纯素厨房都为这门手艺自豪，也乐意讲。",
          "韩国的纯素者仍是个不大而紧密的圈子——店员往往知道城里其他每一家植物性餐厅。",
    ],
    passportMissionZh: { title: "问问厨房", detail: "问问豆制的\"肉\"是用什么做的。韩国的植物性厨房为这门手艺自豪，会很乐意讲给你听。" },
    culturalMeaningZh: "在韩国吃素，比\"纯素\"这个词早了好几个世纪——今天的厨房是在复活一张古老得多的、以蔬菜为主的餐桌。",
    whenKoreansEatThisZh: "越来越是任何一天：过去十年里，首尔的纯素从小众长成了一个正经的品类。",
    didYouKnowKo: "한국의 식물성 식문화를 떠받치는 건 아주 오래된 개념입니다. 나물, 즉 들에서 난 푸성귀를 무쳐 내는 기술이요. 비건이라는 말이 생기기 훨씬 전부터 제대로 차린 한국 밥상은 이미 수십 가지 채소 요리로 이루어져 있었습니다.",
    diningTipsKo: [
          "콩고기 요리는 의심이 들더라도 한 번 드셔 보세요. 한국 주방은 사찰음식을 거치며 수십 년 동안 그 식감을 다듬어 왔습니다.",
          "반찬은 대개 리필됩니다. 더 달라고 하는 건 결례가 아니라 주방에 대한 칭찬이에요."
    ],
    whyLocalsLoveItKo: "서울의 식물성 주방에는 행자와 운동선수와 회의론자가 뒤섞여 앉습니다. 나올 때는 다들 고기 맛이 났는지 아닌지를 두고 다투고요.",
    conversationTipsKo: [
          "콩고기를 무엇으로 만드는지 물어보세요. 서울의 비건 주방 대부분이 그 기술을 자랑스러워하고 설명하기를 좋아합니다.",
          "한국의 비건은 아직 작고 촘촘한 공동체라, 직원분이 도시의 다른 식물성 식당을 거의 다 알고 있는 경우가 많습니다."
    ],
    passportMissionKo: {
          "title": "주방에 물어보기",
          "detail": "콩고기를 무엇으로 만드는지 물어보세요. 한국의 식물성 주방은 그 기술을 자랑스러워하고 기꺼이 설명해 줍니다."
    },
    culturalMeaningKo: "한국에서 식물성 식사는 비건이라는 단어보다 몇 백 년 앞섭니다. 지금의 주방들은 훨씬 오래된 채소 중심의 밥상을 되살리고 있는 셈이에요.",
    whenKoreansEatThisKo: "점점 아무 날이나. 서울의 비건 식문화는 지난 십 년 사이 틈새에서 하나의 분류로 자랐습니다.",
    didYouKnow:
      "Korea's plant-based scene is powered by a very old idea: 나물 (namul), the art of seasoning wild greens. Long before 'vegan' was a word, a proper Korean table was already built around dozens of vegetable dishes.",
    diningTips: [
      "Try the soy-meat dishes even if you're skeptical — Korean kitchens have refined the texture through decades of temple cooking.",
      'Side dishes (banchan) are usually refillable. Asking for more is a compliment to the kitchen, not a faux pas.',
    ],
    whyLocalsLoveIt:
      "Seoul's plant-based kitchens draw a mixed crowd of monks-in-training, athletes and skeptics — everyone leaves arguing about whether it tasted like meat.",
    usefulKorean: [
      { ko: '비건이에요', ro: 'bigeon-ieyo', en: "I'm vegan", ko_gloss: "비건이라고 밝히는 말", es: "Soy vegano", fr: "Je suis végane", ar: "أنا نباتي صِرف", zh: "我吃纯素" },
      { ko: '고기 안 들어가요?', ro: 'gogi an deureogayo?', en: 'Does this have meat in it?', ko_gloss: '고기가 들었는지 묻는 말', es: '¿Lleva carne?', fr: "Est-ce qu'il y a de la viande dedans ?", ar: "هل فيه لحم؟", zh: "这里面有肉吗？" },
      { ko: '두부', ro: 'dubu', en: 'tofu', ko_gloss: "두부", es: "tofu", fr: "tofu", ar: "توفو", zh: "豆腐" },
    ],
    conversationTips: [
      'Ask what the soy-meat is made from — most Seoul vegan kitchens are proud of the technique and love to explain it.',
      "Korean vegans are still a small, tight community — staff often know every other plant-based spot in the city.",
    ],
    passportMission: { title: 'Ask the Kitchen', detail: 'Ask what the soy-meat is made from. Korean plant-based kitchens are proud of the technique and will happily walk you through it.' },
    culturalMeaning: "Plant-based eating in Korea traces back centuries further than the word 'vegan' — modern kitchens are reviving a much older vegetable-forward table.",
    whenKoreansEatThis: "Increasingly, any day — Seoul's vegan scene has grown from a niche to a genuine dining category over the last decade.",
  },
  'halal-korean': {
    didYouKnowEs: "La cocina coreana halal de Seúl creció en Usadan-ro, la calle en cuesta junto a la Mezquita Central: un barrio que acoge a comerciantes y viajeros musulmanes desde los años setenta.",
    diningTipsEs: [
          "El bulgogi y el samgyetang son la entrada más suave a los sabores coreanos: muy sabrosos y sin sobresalto de picante.",
          "La comida coreana es comunitaria: los platos van al centro de la mesa y se comparten.",
          "Busca el certificado de la KMF (Federación Musulmana de Corea) junto a la caja si quieres una garantía halal formal."
    ],
    whyLocalsLoveItEs: "Para los musulmanes que viven en Seúl estas cocinas son menos una novedad que un salvavidas: el único sitio donde una comida coreana casera no exige traducir una lista de ingredientes.",
    conversationTipsEs: [
          "Agradece a la cocina que atienda dietas halal: fuera de Itaewon aún es lo bastante raro como para que el personal lo note y lo agradezca.",
          "Pregunta cuánto lleva la familia con el restaurante; varias de estas cocinas son historias de inmigración de primera generación."
    ],
    passportMissionEs: {
          "title": "Escucha la historia de la familia",
          "detail": "Pregunta cuánto lleva la familia con esta cocina. Muchas son de primera generación, y la respuesta suele ser lo mejor que te llevas a casa."
    },
    culturalMeaningEs: "Estas cocinas son una negociación silenciosa entre la comida casera coreana y la ley alimentaria islámica: la prueba de que nunca estuvieron tan lejos como parecía.",
    whenKoreansEatThisEs: "Para la comunidad musulmana de Seúl esto es sencillamente la cena entre semana; para los visitantes suele ser el primer puente hacia los sabores coreanos.",
    didYouKnowFr: "La cuisine coréenne halal de Séoul a grandi sur Usadan-ro, la rue en pente qui longe la Grande Mosquée de Séoul — un quartier qui accueille commerçants et voyageurs musulmans depuis les années 1970.",
    diningTipsFr: [
          "Le bulgogi et le samgyetang sont les introductions les plus douces aux saveurs coréennes : très savoureuses, sans choc de piment.",
          "La table coréenne est commune : les plats arrivent au milieu et tout le monde partage.",
          "Cherchez le certificat KMF (Korea Muslim Federation) près de la caisse pour une garantie halal formelle.",
    ],
    whyLocalsLoveItFr: "Pour les musulmans qui vivent à Séoul, ces cuisines tiennent moins de la nouveauté que de la bouée : le seul endroit où un repas coréen fait maison n'exige pas de traduire une liste d'ingrédients.",
    conversationTipsFr: [
          "Remerciez la cuisine de servir halal — c'est encore assez rare hors d'Itaewon pour que le personnel le remarque et l'apprécie.",
          "Demandez depuis combien de temps la famille tient le restaurant : plusieurs de ces cuisines sont des histoires d'immigration de première génération, racontées avec fierté.",
    ],
    passportMissionFr: { title: "Écoutez l'histoire de la famille", detail: "Demandez depuis combien de temps la famille tient cette cuisine. Beaucoup sont de première génération, et la réponse est en général la meilleure chose que vous rapporterez." },
    culturalMeaningFr: "Ces cuisines sont une négociation discrète entre la cuisine familiale coréenne et la loi alimentaire islamique — la preuve que les deux n'ont jamais été aussi éloignées qu'on le croyait.",
    whenKoreansEatThisFr: "Pour la communauté musulmane de Séoul, c'est simplement le dîner de semaine ; pour les visiteurs, c'est souvent le premier pont vers les saveurs coréennes.",
    didYouKnowAr: "نشأ مشهد الطعام الكوري الحلال في سول على أوسادان-رو، الشارع المنحدر بجانب مسجد سول المركزي — حيّ يستقبل التجّار والمسافرين المسلمين منذ السبعينيات.",
    diningTipsAr: [
          "البولغوغي والسامغيتانغ أرفق مدخلين إلى النكهات الكورية: عميقة الطعم وبلا صدمة حِرّيف.",
          "المائدة الكورية مشتركة: تصل الأطباق إلى الوسط ويتشارك الجميع.",
          "ابحث عن شهادة KMF (الاتحاد الإسلامي الكوري) قرب الصندوق لضمان حلال رسمي.",
    ],
    whyLocalsLoveItAr: "لمسلمي سول هذه المطابخ ليست طرافة بل طوق نجاة: المكان الوحيد الذي لا تحتاج فيه وجبة كورية بيتيّة إلى ترجمة قائمة مكوّنات.",
    conversationTipsAr: [
          "اشكر المطبخ على مراعاة الحلال — ما زال نادرًا خارج إيتايوون بما يكفي ليلاحظه العاملون ويقدّروه.",
          "اسأل منذ متى تدير العائلة المطعم؛ عدد من هذه المطابخ حكايات هجرة من الجيل الأول تُروى بفخر.",
    ],
    passportMissionAr: { title: "استمع إلى حكاية العائلة", detail: "اسأل منذ متى تدير العائلة هذا المطبخ. كثير منها من الجيل الأول، والجواب عادةً أفضل ما تعود به." },
    culturalMeaningAr: "هذه المطابخ مفاوضة هادئة بين الطبخ البيتي الكوري وأحكام الطعام الإسلامية — دليل أنهما لم يكونا يومًا بالبعد الذي بدا.",
    whenKoreansEatThisAr: "لجالية سول المسلمة هذا ببساطة عشاء يوم أسبوع؛ وللزوّار غالبًا أول جسر إلى النكهات الكورية.",
    didYouKnowZh: "首尔的清真韩餐是在梨泰院中央清真寺旁那条斜坡路乌沙丹路上长起来的——一个从七十年代起就接待穆斯林商人和旅人的街区。",
    diningTipsZh: [
          "烤肉（bulgogi）和参鸡汤是进入韩国味道最温和的两道：味厚，但不会被辣到。",
          "韩国人是一起吃的：菜落在桌子中间，大家一起夹。",
          "想要正式的清真保证，看看柜台附近有没有 KMF（韩国穆斯林联合会）的证书。",
    ],
    whyLocalsLoveItZh: "对住在首尔的穆斯林来说，这些厨房与其说新鲜，不如说是一条命脉：唯一一个吃家常韩餐不必翻译配料表的地方。",
    conversationTipsZh: [
          "为厨房照顾清真饮食道声谢——在梨泰院之外这仍然少见到店员会注意、也会记住。",
          "问问这家店开了多少年；其中好几家是第一代移民的故事，讲起来是有自豪的。",
    ],
    passportMissionZh: { title: "听听这家人的故事", detail: "问问这家人开这个厨房多久了。很多是第一代，而那个答案通常是你带走的最好的东西。" },
    culturalMeaningZh: "这些厨房是韩国家常菜和伊斯兰饮食律法之间一场安静的协商——它们从来没有看上去那么远。",
    whenKoreansEatThisZh: "对首尔的穆斯林社群来说这就是平常的晚饭；对来访的人，往往是通向韩国味道的第一座桥。",
    didYouKnowKo: "서울의 할랄 한식은 서울중앙성원 옆 비탈길, 우사단로에서 자랐습니다. 1970년대부터 무슬림 상인과 여행자를 맞아 온 동네예요.",
    diningTipsKo: [
          "불고기와 삼계탕이 한국 맛으로 들어가는 가장 부드러운 입구입니다. 깊고 구수하며, 매운맛의 충격이 없어요.",
          "한국의 식사는 함께 먹는 방식입니다. 요리가 가운데 놓이고 다 같이 나눠 먹습니다.",
          "공식적인 할랄 확인이 필요하다면 계산대 근처의 KMF(한국이슬람교중앙회) 인증서를 찾아보세요."
    ],
    whyLocalsLoveItKo: "서울에 사는 무슬림들에게 이 주방들은 신기한 곳이 아니라 생명줄에 가깝습니다. 집밥 같은 한식을 먹는 데 성분표 번역이 필요 없는 유일한 자리니까요.",
    conversationTipsKo: [
          "할랄 식단을 챙겨 줘서 고맙다고 말해 보세요. 이태원 밖에서는 아직 드문 일이라 직원분들이 알아채고 반가워합니다.",
          "가족이 이 식당을 얼마나 오래 했는지 물어보세요. 여기 여러 곳이 1세대 이민자의 이야기입니다."
    ],
    passportMissionKo: {
          "title": "가족의 이야기 듣기",
          "detail": "이 주방을 가족이 얼마나 오래 해 왔는지 물어보세요. 여러 곳이 1세대이고, 그 대답이 대개 가장 좋은 기념품이 됩니다."
    },
    culturalMeaningKo: "이 주방들은 한국의 집밥과 이슬람 식이 율법 사이의 조용한 타협을 보여 줍니다. 둘이 보이는 것만큼 멀지 않았다는 증거이기도 하고요.",
    whenKoreansEatThisKo: "서울의 무슬림 공동체에게는 그냥 평일 저녁입니다. 방문객에게는 한국 맛으로 건너오는 첫 다리인 경우가 많고요.",
    didYouKnow:
      "Seoul's halal Korean food scene grew up on Usadan-ro, the sloped street beside the Seoul Central Mosque — a neighborhood that has welcomed Muslim traders and travelers since the 1970s.",
    diningTips: [
      'Bulgogi and samgyetang are the gentlest introductions to Korean flavors — deeply savory, no spice shock.',
      'Korean dining is communal: dishes land in the middle of the table and everyone shares.',
      'Look for the KMF (Korea Muslim Federation) certificate near the counter for formal halal assurance.',
    ],
    whyLocalsLoveIt:
      "For Seoul's Muslim residents, these kitchens are less a novelty and more a lifeline — the one place a home-style Korean meal doesn't require a translated ingredient list.",
    usefulKorean: [
      { ko: '할랄 음식 있어요?', ro: 'hallal eumsik isseoyo?', en: 'Do you have halal food?', ko_gloss: '할랄 음식이 있는지 묻는 말', es: '¿Tienen comida halal?', fr: "Avez-vous de la nourriture halal ?", ar: "هل لديكم طعام حلال؟", zh: "你们有清真食品吗？" },
      { ko: '돼지고기 안 들어가요', ro: 'dwaejigogi an deureogayo', en: 'No pork in this', ko_gloss: "돼지고기가 없다는 말", es: "Esto no lleva cerdo", fr: "Il n'y a pas de porc dedans", ar: "لا يوجد فيه لحم خنزير", zh: "这里面没有猪肉" },
      { ko: '감사합니다', ro: 'gamsahamnida', en: 'Thank you', ko_gloss: "고맙다는 인사", es: "Gracias", fr: "Merci", ar: "شكرًا", zh: "谢谢" },
    ],
    conversationTips: [
      "Thank the kitchen for catering to halal diets — it's still rare enough outside Itaewon that staff notice and appreciate it.",
      'Ask how long the family has run the restaurant; several of these kitchens are first-generation, proud immigrant stories.',
    ],
    passportMission: { title: 'Hear the Family Story', detail: 'Ask how long the family has run this kitchen. Many are first-generation, and the answer is usually the best thing you take home.' },
    culturalMeaning: 'These kitchens represent a quiet negotiation between Korean home cooking and Islamic dietary law — proof the two were never as far apart as they seemed.',
    whenKoreansEatThis: "For Seoul's Muslim community, this is simply weekday dinner — for visitors, it's often a first bridge into Korean flavors.",
  },
  'world-halal': {
    didYouKnowEs: "Incheon lleva más de un siglo siendo la puerta de entrada de la comida global a Corea: su puerto abrió el primer barrio chino del país, y hoy sus cocinas halal alimentan a uno de los distritos más internacionales de Corea.",
    diningTipsEs: [
          "Las raciones están pensadas para compartir: pide varios platos para la mesa, al estilo coreano.",
          "Muchos locales sirven encurtidos coreanos junto al curry y el kebab, una pequeña fusión local que vale la pena probar."
    ],
    whyLocalsLoveItEs: "Las cocinas halal de Incheon alimentan a un barrio genuinamente internacional: aquí es tan probable sentarte junto a un habitual coreano como junto a otro viajero.",
    conversationTipsEs: [
          "Pregunta en qué se diferencia la versión de Incheon de la de su país de origen: las cocinas de ciudad portuaria suelen adaptar las recetas a los ingredientes locales.",
          "La escena halal de Incheon es lo bastante pequeña como para que los dueños se conozcan entre sí: pide otra recomendación cerca."
    ],
    passportMissionEs: {
          "title": "Pregunta por el siguiente",
          "detail": "Los dueños halal de Incheon suelen conocerse. Pregunta dónde comen en su día libre y ve allí."
    },
    culturalMeaningEs: "Las cocinas halal de Incheon son la prueba viva del oficio de una ciudad portuaria: absorber la comida del mundo y hacerle sitio en la mesa.",
    whenKoreansEatThisEs: "La hora de comer de los trabajadores y estudiantes internacionales que forman los distritos más diversos de Incheon.",
    didYouKnowFr: "Incheon est depuis plus d'un siècle la porte d'entrée de la nourriture du monde en Corée : son port a ouvert le premier quartier chinois du pays, et ses cuisines halal nourrissent aujourd'hui l'un des districts les plus internationaux de Corée.",
    diningTipsFr: [
          "Les portions sont faites pour être partagées : commandez quelques plats pour la table, à la coréenne.",
          "Beaucoup de boutiques servent des pickles à la coréenne à côté du curry et du kebab — une petite habitude de fusion locale qui vaut le détour.",
    ],
    whyLocalsLoveItFr: "Les cuisines halal d'Incheon nourrissent un quartier réellement international : ici vous avez autant de chances d'être assis à côté d'un habitué coréen que d'un autre voyageur.",
    conversationTipsFr: [
          "Demandez ce que la version d'Incheon change par rapport au pays d'origine — les cuisines de ville portuaire adaptent souvent les recettes aux ingrédients locaux.",
          "La scène halal d'Incheon est assez petite pour que les patrons se connaissent : demandez une autre adresse dans le coin.",
    ],
    passportMissionFr: { title: "Demandez la suivante", detail: "Les patrons halal d'Incheon se connaissent en général. Demandez où ils mangent leur jour de congé, et allez-y." },
    culturalMeaningFr: "Les cuisines halal d'Incheon sont la preuve vivante du métier d'une ville portuaire : absorber la nourriture du monde et lui faire une place à table.",
    whenKoreansEatThisFr: "La pause déjeuner des travailleurs et étudiants internationaux qui composent les quartiers les plus divers d'Incheon.",
    didYouKnowAr: "ظلّت إنتشون أكثر من قرن بوّابة كوريا إلى طعام العالم: فتح مرفؤها أول حيّ صيني في البلاد، وتطعم مطابخها الحلال اليوم واحدًا من أكثر أحيائها عالميّة.",
    diningTipsAr: [
          "الحصص مصنوعة للمشاركة: اطلب بضعة أطباق للطاولة، على الطريقة الكورية.",
          "كثير من المحلات تقدّم مخلّلات على الطريقة الكورية بجانب الكاري والكباب — عادة اندماج محلية صغيرة تستحق التجربة.",
    ],
    whyLocalsLoveItAr: "تطعم مطابخ إنتشون الحلال حيًّا عالميًّا حقًّا: تطلب هنا فتجلس بالقدر نفسه بجانب مداوم كوري أو مسافر مثلك.",
    conversationTipsAr: [
          "اسأل ما الذي يختلف في نسخة إنتشون عن نسخة البلد الأصلي — مطابخ مدن الموانئ تكيّف الوصفات مع المتوفّر.",
          "المشهد الحلال في إنتشون صغير بما يكفي ليعرف أصحابه بعضهم: اطلب اقتراحًا آخر قريبًا.",
    ],
    passportMissionAr: { title: "اسأل عن التالي", detail: "أصحاب المحلات الحلال في إنتشون يعرفون بعضهم عادةً. اسأل أين يأكلون في يوم عطلتهم، ثم اذهب." },
    culturalMeaningAr: "مطابخ إنتشون الحلال دليل حيّ على وظيفة مدينة ميناء: أن تستوعب طعام العالم وتفسح له مكانًا على المائدة.",
    whenKoreansEatThisAr: "استراحة غداء العمّال والطلبة الدوليين الذين يصنعون أكثر أحياء إنتشون تنوّعًا.",
    didYouKnowZh: "一百多年来仁川一直是世界食物进入韩国的门：它的港口开出了全国第一个中华街，而今天它的清真厨房喂着韩国最国际化的街区之一。",
    diningTipsZh: [
          "份量是给人分着吃的：按韩国的方式，给这一桌点几样。",
          "不少店会在咖喱和烤肉旁边配韩式泡菜小碟——一个值得一试的本地融合习惯。",
    ],
    whyLocalsLoveItZh: "仁川的清真厨房喂着一个真正国际化的街区：在这里点单，坐在你旁边的可能是韩国常客，也可能是另一个旅人。",
    conversationTipsZh: [
          "问问仁川这版和家乡那版有什么不一样——港口城市的厨房常常按本地食材改配方。",
          "仁川的清真圈子小到老板们彼此都认识——顺口问一家附近的。",
    ],
    passportMissionZh: { title: "问下一家", detail: "仁川做清真的老板们通常互相认识。问问他们休息那天在哪儿吃，然后照着去。" },
    culturalMeaningZh: "仁川的清真厨房是一座港口城市职责的活证据：把世界的食物吸收进来，并在桌上给它腾出位置。",
    whenKoreansEatThisZh: "构成仁川最多元街区的国际工人和学生的午休时间。",
    didYouKnowKo: "인천은 백 년 넘게 한국이 세계의 음식을 받아들이는 관문이었습니다. 이 항구가 한국 최초의 차이나타운을 열었고, 지금은 그 할랄 주방들이 한국에서 가장 국제적인 동네를 먹이고 있어요.",
    diningTipsKo: [
          "양이 나눠 먹기에 맞춰져 있습니다. 한국식으로 여러 가지를 상에 시키세요.",
          "카레나 케밥 옆에 한국식 절임을 함께 내는 집이 많습니다. 작지만 해볼 만한 현지식 조합이에요."
    ],
    whyLocalsLoveItKo: "인천의 할랄 주방은 진짜 국제적인 동네를 먹입니다. 여기서 주문하면 옆자리에 앉는 사람이 여행자일 확률만큼이나 한국인 단골일 확률도 높아요.",
    conversationTipsKo: [
          "같은 요리가 본국의 것과 인천 버전이 어떻게 다른지 물어보세요. 항구 도시의 주방은 현지 재료에 맞춰 조리법을 바꾸는 일이 많습니다.",
          "인천의 할랄 식당가는 사장님들끼리 서로 아는 정도로 작습니다. 근처에 또 어디가 좋은지 추천을 청해 보세요."
    ],
    passportMissionKo: {
          "title": "다음 집 물어보기",
          "detail": "인천의 할랄 사장님들은 서로를 압니다. 쉬는 날에 어디서 드시는지 묻고, 그대로 따라가 보세요."
    },
    culturalMeaningKo: "인천의 할랄 주방은 항구 도시가 하는 일을 그대로 보여 줍니다. 세계의 음식을 받아들이고, 그것에 자리를 내주는 일이요.",
    whenKoreansEatThisKo: "인천에서 가장 다양한 동네를 이루는 외국인 노동자와 학생들의 점심시간.",
    didYouKnow:
      "Incheon has been Korea's gateway for global food for over a century — its port opened the country's first Chinatown, and today its halal kitchens feed one of Korea's most international districts.",
    diningTips: [
      'Portions are made for sharing — order a few dishes for the table, Korean style.',
      'Many shops serve Korean-style pickles alongside curry and kebab — a small local fusion habit worth trying.',
    ],
    whyLocalsLoveIt:
      "Incheon's halal kitchens feed a genuinely international neighborhood — order here and you're as likely to sit next to a Korean regular as a fellow traveler.",
    usefulKorean: [
      { ko: '이거 매워요?', ro: 'igeo maewoyo?', en: 'Is this spicy?', ko_gloss: '매운지 묻는 말', es: '¿Esto pica?', fr: "Est-ce que c'est piquant ?", ar: "هل هذا حارّ؟", zh: "这个辣吗？" },
      { ko: '포장이요', ro: 'pojang-iyo', en: 'To go, please', ko_gloss: "포장해 달라는 말", es: "Para llevar, por favor", fr: "À emporter, s'il vous plaît", ar: "سفري، من فضلك", zh: "打包，谢谢" },
      { ko: '물 좀 주세요', ro: 'mul jom juseyo', en: 'Water, please', ko_gloss: "물을 달라는 말", es: "Agua, por favor", fr: "De l'eau, s'il vous plaît", ar: "ماء، من فضلك", zh: "请给我水" },
    ],
    conversationTips: [
      "Ask what's different about the Incheon version of the dish versus back home — port-city kitchens often adapt recipes for local ingredients.",
      "Incheon's halal scene is small enough that owners often know each other — ask for another recommendation nearby.",
    ],
    passportMission: { title: 'Ask for the Next One', detail: "Incheon's halal owners tend to know each other. Ask where they eat on their day off, and follow it." },
    culturalMeaning: "Incheon's halal kitchens are living proof of a port city's job: absorbing the world's food and making room for it at the table.",
    whenKoreansEatThis: "Lunch break for the international workers and students who make up Incheon's most diverse districts.",
  },
  'zero-waste': {
    didYouKnowEs: "Corea recicla cerca del 95% de sus residuos de comida, y separarlos es obligatorio desde 2013: los hogares pagan por peso, con bolsas designadas o un contenedor con tarjeta. Los cafés sin residuos van un paso más allá: nada desechable cruza el mostrador, para empezar.",
    diningTipsEs: [
          "Lleva tu propio vaso o recipiente: casi todas las tiendas sin residuos hacen un pequeño descuento.",
          "La cocina de raíz a hoja hace que aparezcan en el plato partes de la verdura que no sueles comer. Fíate de la cocina."
    ],
    whyLocalsLoveItEs: "Los habituales traen sus propios recipientes sin que nadie se lo pida: aquí es menos un argumento de marketing que una costumbre que el barrio ha adoptado de verdad.",
    conversationTipsEs: [
          "Pregunta qué pasa con los restos de comida: el sistema coreano de reciclaje sorprende a casi todos los que llegan por primera vez.",
          "Lleva un vaso reutilizable si tienes uno; la mayoría de estos sitios lo premian discretamente con un descuento."
    ],
    passportMissionEs: {
          "title": "No dejes nada",
          "detail": "Rechaza un desechable: un vaso, una bolsa, una pajita. Y ya que estás, pregunta qué pasa con los restos de comida."
    },
    culturalMeaningEs: "Comer sin residuos recoge un instinto muy coreano de ahorro y aprovechamiento, apuntado ahora al clima y no solo al bolsillo.",
    whenKoreansEatThisEs: "Siempre que alguien quiere que su comida deje atrás lo menos posible: cada vez más una elección diaria, no una ocasión especial.",
    didYouKnowFr: "La Corée recycle environ 95 % de ses déchets alimentaires, et le tri est obligatoire depuis 2013 : les foyers paient au poids, par sacs désignés ou par bac à carte. Les cafés zéro déchet vont un cran plus loin : rien de jetable ne passe le comptoir, au départ.",
    diningTipsFr: [
          "Apportez votre gourde ou votre contenant — la plupart des boutiques zéro déchet accordent une petite remise pour cela.",
          "La cuisine « de la racine à la feuille » veut dire que des parties inhabituelles des légumes peuvent arriver dans votre assiette. Faites confiance à la cuisine.",
    ],
    whyLocalsLoveItFr: "Les habitués apportent leurs propres contenants sans qu'on le leur demande : ici c'est moins un argument marketing qu'une habitude que le quartier a vraiment prise.",
    conversationTipsFr: [
          "Demandez ce que deviennent les restes — le système coréen de recyclage des déchets alimentaires surprend la plupart des visiteurs.",
          "Apportez une tasse réutilisable si vous en avez une : la plupart des adresses zéro déchet la récompensent discrètement d'une petite remise.",
    ],
    passportMissionFr: { title: "Ne laissez rien", detail: "Refusez un objet jetable — gobelet, sac, paille. Et tant que vous y êtes, demandez ce que deviennent les restes." },
    culturalMeaningFr: "Le repas zéro déchet reprend un instinct très coréen d'économie et de débrouille, tourné maintenant vers le climat et plus seulement vers le porte-monnaie.",
    whenKoreansEatThisFr: "Chaque fois que quelqu'un veut que son repas laisse le moins possible derrière lui — de plus en plus un choix quotidien, pas une occasion spéciale.",
    didYouKnowAr: "تعيد كوريا تدوير نحو 95% من نفاياتها الغذائية، وفرزها إلزامي منذ 2013: تدفع البيوت بالوزن، عبر أكياس مخصّصة أو حاوية تعمل ببطاقة. والمقاهي بلا نفايات تخطو خطوة أبعد: لا يعبر شيء يُرمى المنضدة أصلًا.",
    diningTipsAr: [
          "أحضِر كوبك أو وعاءك — معظم محلات بلا نفايات تمنح خصمًا صغيرًا عليه.",
          "الطبخ «من الجذر إلى الورقة» يعني أن أجزاء غير مألوفة من الخضار قد تصل إلى صحنك. ثِق بالمطبخ.",
    ],
    whyLocalsLoveItAr: "يحضر المداومون أوعيتهم دون أن يُطلب منهم: الأمر هنا عادة أخذها الحيّ فعلًا لا زاوية تسويق.",
    conversationTipsAr: [
          "اسأل ما مصير بقايا الطعام — نظام إعادة تدوير النفايات الغذائية في كوريا يفاجئ معظم الزوّار لأول مرة.",
          "أحضِر كوبًا قابلًا لإعادة الاستعمال إن كان معك؛ معظم الأماكن تكافئه بهدوء بخصم صغير.",
    ],
    passportMissionAr: { title: "لا تترك شيئًا", detail: "ارفض شيئًا واحدًا يُرمى — كوبًا أو كيسًا أو قشّة. واسأل ما مصير بقايا الطعام ما دمت هناك." },
    culturalMeaningAr: "يعكس الأكل بلا نفايات غريزة كورية عريقة في الاقتصاد وحسن التصرّف، مصوّبة الآن نحو المناخ لا نحو الجيب وحده.",
    whenKoreansEatThisAr: "كلّما أراد آكل أن تترك وجبته أقلّ ما يمكن خلفها — خيار يومي متزايد لا مناسبة خاصة.",
    didYouKnowZh: "韩国大约95%的厨余会被回收，而从2013年起分类是强制的：家庭按重量付费，用指定袋或刷卡的回收桶。零废弃咖啡馆再往前一步：一开始就不让一次性的东西越过柜台。",
    diningTipsZh: [
          "带上你自己的杯子或容器——多数零废弃的店会为此少收你一点。",
          "\"从根到叶\"的做法意味着蔬菜上不常见的部位可能出现在你盘子里。相信厨房。",
    ],
    whyLocalsLoveItZh: "常客不用人说就自己带容器来——在这里这不太算营销角度，更像是这个街区真的养成的习惯。",
    conversationTipsZh: [
          "问问厨余最后去了哪儿——韩国的厨余回收制度会让多数第一次来的人吃惊。",
          "有可重复使用的杯子就带上；多数零废弃的店会不声不响地少收一点。",
    ],
    passportMissionZh: { title: "什么都别留下", detail: "拒绝一样一次性的东西——杯子、袋子或吸管。顺便问问厨余最后去了哪儿。" },
    culturalMeaningZh: "零废弃的吃法呼应着一种很韩国的节俭和会过日子的本能，只是如今对准的是气候，而不只是钱包。",
    whenKoreansEatThisZh: "任何时候，只要有人希望自己这顿饭尽量少留下东西——这越来越是日常的选择，而不是特殊场合。",
    didYouKnowKo: "한국은 음식물 쓰레기의 약 95%를 재활용하고, 2013년부터 분리배출이 의무입니다. 가정은 전용 봉투나 카드형 수거함으로 무게만큼 값을 냅니다. 제로웨이스트 카페는 한 걸음 더 나아가, 애초에 일회용품이 계산대를 넘어오지 않게 합니다.",
    diningTipsKo: [
          "텀블러나 용기를 들고 가세요. 대부분의 제로웨이스트 가게가 작은 할인을 해 줍니다.",
          "뿌리부터 잎까지 쓰는 조리법이라, 평소에 안 먹던 부위가 접시에 올라올 수 있습니다. 주방을 믿어 보세요."
    ],
    whyLocalsLoveItKo: "단골들은 말하지 않아도 자기 용기를 들고 옵니다. 여기서는 마케팅 문구라기보다 동네가 실제로 들인 습관에 가까워요.",
    conversationTipsKo: [
          "음식물 찌꺼기가 어디로 가는지 물어보세요. 한국의 음식물 재활용 체계는 처음 온 사람 대부분을 놀라게 합니다.",
          "다회용 컵이 있다면 들고 가세요. 대부분의 제로웨이스트 가게가 조용히 할인으로 답합니다."
    ],
    passportMissionKo: {
          "title": "아무것도 남기지 않기",
          "detail": "일회용품 하나를 거절해 보세요 — 컵, 봉투, 빨대 중 하나. 겸사겸사 음식물 찌꺼기가 어디로 가는지도 물어보시고요."
    },
    culturalMeaningKo: "제로웨이스트 식사에는 아끼고 남김없이 쓰는 아주 한국적인 감각이 들어 있습니다. 이제 그 대상이 지갑이 아니라 기후일 뿐이에요.",
    whenKoreansEatThisKo: "한 끼가 남기는 것을 최대한 줄이고 싶을 때. 특별한 날이 아니라 점점 일상의 선택이 되고 있습니다.",
    didYouKnow:
      // The one hard number in this file. Checked against
      // wikipedia-recycling-kr in src/content/sources.js: 95%, and separating
      // food waste has been mandatory since 2013. That source documents
      // biodegradable bags, with RFID bins in Seoul, so the sentence no longer
      // says every household uses bins.
      'Korea recycles about 95% of its food waste, and separating it has been mandatory since 2013 — households pay by weight, through designated bags or a card-operated bin. Zero-waste cafes take the next step: nothing disposable crosses the counter in the first place.',
    diningTips: [
      'Bring your own tumbler or container — most zero-waste shops offer a small discount for it.',
      'Root-to-leaf cooking means unfamiliar parts of vegetables may appear on your plate. Trust the kitchen.',
    ],
    whyLocalsLoveIt:
      "Regulars bring their own containers without being asked — it's less a marketing angle here and more an actual habit the neighborhood has picked up.",
    usefulKorean: [
      { ko: '텀블러 있어요', ro: 'teombeulleo isseoyo', en: 'I have my own tumbler/cup', ko_gloss: "텀블러를 가져왔다는 말", es: "Traigo mi propio vaso", fr: "J'ai ma propre tasse", ar: "معي كوبي الخاص", zh: "我自己带了杯子" },
      { ko: '일회용품 필요 없어요', ro: 'ilhoeyongpum piryo eopseoyo', en: "I don't need disposables", ko_gloss: "일회용품이 필요 없다는 말", es: "No necesito desechables", fr: "Je n'ai pas besoin de jetable", ar: "لا أحتاج إلى أدوات تُرمى", zh: "我不需要一次性用品" },
      { ko: '잘 먹었습니다', ro: 'jal meogeotseumnida', en: 'That was a good meal (said after eating)', ko_gloss: '식사 후에 하는 인사', es: 'He comido muy bien (se dice al terminar)', fr: "C'était un bon repas (se dit après avoir mangé)", ar: "كانت وجبة طيّبة (تُقال بعد الأكل)", zh: "这顿饭很好（吃完说的）" },
    ],
    conversationTips: [
      "Ask what happens to the food scraps — Korea's food-waste recycling system surprises most first-time visitors.",
      'Bring a reusable cup if you have one; most zero-waste spots quietly reward it with a small discount.',
    ],
    passportMission: { title: 'Leave Nothing Behind', detail: 'Refuse one disposable item — cup, bag, straw. Ask what happens to the food scraps while you are at it.' },
    culturalMeaning: 'Zero-waste dining reflects a very Korean instinct for thrift and resourcefulness, now aimed at the climate instead of just the wallet.',
    whenKoreansEatThis: 'Whenever a diner wants their meal to leave as little behind as it can — a growing everyday choice, not a special occasion.',
  },
  'brunch-bakery': {
    didYouKnowEs: "Seúl tiene una de las densidades de cafeterías más altas del mundo, y el brunch de fin de semana está más cerca de un ritual que de una comida. La versión coreana se apoya en verdura de temporada y horneado lento más que en salsas pesadas.",
    diningTipsEs: [
          "Las cafeterías son sitios para quedarse: una consumición te compra cómodamente la silla toda la tarde.",
          "Pregunta qué se ha horneado hoy; las panaderías de producción corta agotan lo mejor antes del mediodía."
    ],
    whyLocalsLoveItEs: "Seúl trata el brunch del fin de semana como un destino, no como una comida: la gente cruza la ciudad por una panadería que la semana pasada agotó a mediodía.",
    conversationTipsEs: [
          "Pregunta qué se ha horneado esta mañana: las panaderías pequeñas cambian el surtido a diario y agradecen la pregunta.",
          "Aquí la cultura de café es de quedarse; nadie te echará después de una consumición."
    ],
    passportMissionEs: {
          "title": "Horneado esta mañana",
          "detail": "Pregunta qué ha salido hoy del horno y pide eso en lugar de lo que tenías pensado. El surtido cambia a diario y rara vez llega a la carta."
    },
    culturalMeaningEs: "La cafetería de Seúl es una institución social por derecho propio: un sitio para dejarse ver, trabajar y quedarse, construido sobre tradiciones de panadería tomadas prestadas y rehechas.",
    whenKoreansEatThisEs: "A media mañana del fin de semana, casi como un rito: es la comida más lenta y deliberada de la semana.",
    didYouKnowFr: "Séoul a l'une des plus fortes densités de cafés au monde, et le brunch du week-end tient davantage du rituel que du repas. La version coréenne s'appuie sur les légumes de saison et une cuisson lente plutôt que sur des sauces lourdes.",
    diningTipsFr: [
          "Les cafés sont des lieux où l'on s'attarde en Corée : une consommation vous achète confortablement la place pour l'après-midi.",
          "Demandez ce qui a été cuit aujourd'hui ; les boulangeries en petites fournées vendent leurs meilleures pièces avant midi.",
    ],
    whyLocalsLoveItFr: "Séoul traite le brunch du week-end comme une destination, pas comme un repas : les gens d'ici traversent la ville pour une boulangerie qui était en rupture à midi la semaine dernière.",
    conversationTipsFr: [
          "Demandez ce qui est sorti du four ce matin — les boulangeries en petites fournées changent d'assortiment chaque jour et adorent qu'on le leur demande.",
          "On s'attarde, ici : personne ne vous poussera dehors après une seule consommation.",
    ],
    passportMissionFr: { title: "Sorti du four ce matin", detail: "Demandez ce qui est sorti du four aujourd'hui et prenez cela plutôt que ce que vous aviez prévu. L'assortiment change chaque jour et figure rarement sur la carte." },
    culturalMeaningFr: "Le café de Séoul est une institution sociale à part entière — un lieu où être vu, où travailler, où s'attarder — bâti sur des traditions de boulangerie empruntées puis refaites.",
    whenKoreansEatThisFr: "Les fins de matinée du week-end, presque rituellement : c'est le repas le plus lent et le plus délibéré de la semaine.",
    didYouKnowAr: "في سول واحدة من أعلى كثافات المقاهي على الأرض، وفطور العطلة المتأخر أقرب إلى طقس منه إلى وجبة. والنسخة الكورية تتّكئ على خضار الموسم والخبز البطيء لا على الصلصات الثقيلة.",
    diningTipsAr: [
          "المقاهي في كوريا أماكن تمهّل: مشروب واحد يشتري لك المقعد لبعد الظهر مرتاحًا.",
          "اسأل ما الذي خُبز اليوم؛ المخابز الصغيرة تبيع أفضل أصنافها قبل الظهر.",
    ],
    whyLocalsLoveItAr: "تعامل سول فطور العطلة المتأخر كوجهة لا كوجبة: يعبر أهل المكان المدينة من أجل مخبز نفد ما فيه قبل الظهر الأسبوع الماضي.",
    conversationTipsAr: [
          "اسأل ما الذي خُبز طازجًا هذا الصباح — المخابز الصغيرة تغيّر أصنافها يوميًا وتحبّ أن تُسأل.",
          "ثقافة التمهّل قائمة هنا؛ لن يستعجلك أحد بعد مشروب واحد.",
    ],
    passportMissionAr: { title: "خُبز هذا الصباح", detail: "اسأل ما الذي خرج من الفرن اليوم واطلبه بدل ما كنت تنوي. الأصناف تتغيّر كل يوم ونادرًا ما تصل إلى القائمة." },
    culturalMeaningAr: "مقهى سول مؤسسة اجتماعية قائمة بذاتها — مكان لتُرى فيه، ولتعمل، ولتتمهّل — بُني حول تقاليد خَبز استُعيرت وأُعيدت صناعتها.",
    whenKoreansEatThisAr: "ضحى العطلات، على نحو شبه طقسي: إنه أبطأ وجبات الأسبوع وأكثرها تأنّيًا.",
    didYouKnowZh: "首尔的咖啡馆密度是地球上最高的之一，而周末的早午饭更接近仪式而不是一顿饭。韩国的版本靠时令蔬菜和慢烘焙，而不是浓重的酱汁。",
    diningTipsZh: [
          "咖啡馆在韩国是可以久坐的地方：一杯就足够让那个位子归你一个下午。",
          "问问今天烤了什么；小批量的面包房，最好的东西中午之前就卖完了。",
    ],
    whyLocalsLoveItZh: "首尔把周末的早午饭当作目的地而不是一顿饭——本地人会为一家上周中午就卖光的面包房横穿整座城市。",
    conversationTipsZh: [
          "问问今早新烤的是什么——小批量的面包房每天换品项，也乐意被问。",
          "这里是可以久坐的文化；喝完一杯不会有人催你走。",
    ],
    passportMissionZh: { title: "今早烤的", detail: "问问今天出炉的是什么，点那个，而不是你原本打算点的。品项每天都变，而且很少写进菜单。" },
    culturalMeaningZh: "首尔的咖啡馆本身就是一种社会机构——一个被看见、工作、消磨时间的地方——建立在借来又重新做过的烘焙传统之上。",
    whenKoreansEatThisZh: "周末的上午，几乎是仪式性的：那是一周里最慢、最从容的一顿。",
    didYouKnowKo: "서울은 지구에서 카페 밀도가 가장 높은 도시 중 하나이고, 주말 브런치는 끼니라기보다 의식에 가깝습니다. 한국식은 무거운 소스보다 제철 채소와 느린 베이킹 쪽으로 기울어요.",
    diningTipsKo: [
          "한국에서 카페는 머무는 공간입니다. 한 잔이면 오후 내내 그 자리가 편안히 내 것이 돼요.",
          "오늘 무엇을 구웠는지 물어보세요. 소량으로 굽는 빵집은 제일 좋은 것이 점심 전에 동납니다."
    ],
    whyLocalsLoveItKo: "서울은 주말 브런치를 끼니가 아니라 목적지로 여깁니다. 지난주에 정오 전에 동난 빵집이라면 도시를 가로질러 가기도 해요.",
    conversationTipsKo: [
          "오늘 아침에 새로 구운 게 뭔지 물어보세요. 소량으로 굽는 빵집은 매일 구성이 바뀌고, 그 질문을 반가워합니다.",
          "여기 카페는 오래 머무는 문화입니다. 한 잔 마셨다고 나가라고 하는 사람은 없어요."
    ],
    passportMissionKo: {
          "title": "오늘 아침에 구운 것",
          "detail": "오늘 오븐에서 나온 게 뭔지 묻고, 원래 시키려던 것 대신 그걸 주문해 보세요. 구성은 매일 바뀌고 메뉴판에는 잘 오르지 않습니다."
    },
    culturalMeaningKo: "서울의 카페는 그 자체로 하나의 사회적 제도입니다. 보이고, 일하고, 머무는 자리이며, 빌려 와 다시 만든 제빵 전통 위에 서 있어요.",
    whenKoreansEatThisKo: "주말 늦은 아침, 거의 의식처럼. 한 주에서 가장 느리고 가장 공들인 한 끼가 그때 일어납니다.",
    didYouKnow:
      "Seoul has one of the highest cafe densities on earth, and the weekend brunch is closer to a ritual than a meal. Korea's version leans on seasonal vegetables and slow baking rather than heavy sauces.",
    diningTips: [
      'Cafes are lingering spaces in Korea — one drink comfortably buys you the seat for the afternoon.',
      'Ask what was baked today; small-batch bakeries sell out of their best items before noon.',
    ],
    whyLocalsLoveIt:
      "Seoul treats weekend brunch as a destination, not a meal — locals will cross the city for a bakery that sold out by noon last week.",
    usefulKorean: [
      { ko: '오늘 뭐가 맛있어요?', ro: 'oneul mwoga masisseoyo?', en: "What's good today?", ko_gloss: '오늘 뭐가 좋은지 묻는 말', es: '¿Qué está bueno hoy?', fr: "Qu'est-ce qui est bon aujourd'hui ?", ar: "ما الطيّب اليوم؟", zh: "今天什么好？" },
      { ko: '여기서 먹을게요', ro: 'yeogiseo meogeulgeyo', en: "I'll eat here (not takeout)", ko_gloss: '매장에서 먹겠다는 말', es: 'Voy a comer aquí (no para llevar)', fr: "Je mange sur place (pas à emporter)", ar: "سآكل هنا (لا سفري)", zh: "我在这儿吃（不打包）" },
      { ko: '커피 한 잔이요', ro: 'keopi han janiyo', en: 'One coffee, please', ko_gloss: "커피 한 잔 주문", es: "Un café, por favor", fr: "Un café, s'il vous plaît", ar: "قهوة واحدة، من فضلك", zh: "一杯咖啡，谢谢" },
    ],
    conversationTips: [
      'Ask what was baked fresh this morning — small-batch bakeries change their lineup daily and love being asked.',
      "Cafes are a lingering culture here; nobody will rush you out after one drink.",
    ],
    passportMission: { title: 'Baked This Morning', detail: 'Ask what came out of the oven today and order that instead of what you planned. The lineup changes daily and rarely makes the menu.' },
    culturalMeaning: 'The Seoul cafe is a social institution in its own right — a place to be seen, to work, to linger, built around baking traditions borrowed and remade.',
    whenKoreansEatThis: "Weekend late mornings, almost ritually — it's when the week's slowest, most deliberate meal happens.",
  },
  'local-seasonal': {
    didYouKnowEs: "La cocina coreana sigue el 제철, \"el giro de la estación\". Las cartas cambian sin ruido según entran los ingredientes de temporada, y por eso los locales preguntan \"¿qué está bueno hoy?\" en vez de leer el menú.",
    diningTipsEs: [
          "Pregunta qué es de temporada: el mejor plato muchas veces no está en la carta impresa.",
          "Con un recorrido tan corto, la verdura de tu plato se recogió probablemente hace un día o dos."
    ],
    whyLocalsLoveItEs: "Los habituales no piden la carta: preguntan qué es de temporada, porque la cocina lo cambia antes de que la versión impresa se entere.",
    conversationTipsEs: [
          "Pregunta qué se ha recogido cerca esta semana: el 제철 es un motivo de orgullo local muy real.",
          "Si un plato no está en la carta impresa, pregunta igualmente: lo mejor de temporada casi nunca aparece escrito."
    ],
    passportMissionEs: {
          "title": "Que pida la estación",
          "detail": "Pregunta 제철 음식이 뭐예요? y pide lo que te contesten, aunque todavía no sepas pronunciarlo."
    },
    culturalMeaningEs: "Comer con el 제철 convierte el calendario en la carta de verdad: comer al paso de lo que la tierra está dando ahora mismo.",
    whenKoreansEatThisEs: "Cada vez que gira la estación: el plato que hay hoy en tu mesa no estará dentro de seis semanas.",
    didYouKnowFr: "La cuisine coréenne suit le 제철 (jecheol), « le tour de la saison ». Les cartes changent discrètement à mesure que les produits arrivent, et c'est pourquoi les gens d'ici demandent « qu'est-ce qui est bon aujourd'hui ? » au lieu de lire le menu.",
    diningTipsFr: [
          "Demandez ce qui est de saison — le meilleur plat n'est souvent pas sur la carte imprimée.",
          "Avec des circuits aussi courts, les légumes de votre assiette ont sans doute été récoltés il y a un jour ou deux.",
    ],
    whyLocalsLoveItFr: "Les habitués ne demandent pas la carte : ils demandent ce qui est de saison, parce que la cuisine change avant que la version imprimée ne suive.",
    conversationTipsFr: [
          "Demandez ce qui est récolté dans le coin cette semaine — le 제철 (jecheol, « le tour de la saison ») est une vraie fierté locale.",
          "Si un plat n'est pas sur la carte imprimée, demandez quand même : le meilleur produit de saison n'y figure souvent pas.",
    ],
    passportMissionFr: { title: "Laissez la saison commander", detail: "Demandez 제철 음식이 뭐예요? et prenez ce qu'on vous répond — même si vous ne savez pas encore le prononcer." },
    culturalMeaningFr: "Manger 제철 (jecheol), c'est traiter le calendrier comme la vraie carte : manger au pas de ce que la terre produit en ce moment.",
    whenKoreansEatThisFr: "À chaque tour de saison — le plat qui est sur votre table ce mois-ci n'y sera plus dans six semaines.",
    didYouKnowAr: "يتبع الطبخ الكوري الـ제철 (جيتشول)، أي «دورة الموسم». تتغيّر القوائم بهدوء كلّما دخل محصول موسمه، ولهذا يسأل أهل المكان «ما الطيّب اليوم؟» بدل قراءة القائمة.",
    diningTipsAr: [
          "اسأل عمّا هو في موسمه — أفضل طبق غالبًا ليس على القائمة المطبوعة.",
          "مع مسافات نقل بهذا القِصَر، غالبًا حُصدت خضار صحنك قبل يوم أو يومين.",
    ],
    whyLocalsLoveItAr: "لا يطلب المداومون القائمة: يسألون عمّا هو في موسمه، لأن المطبخ يغيّره قبل أن تلحق به النسخة المطبوعة.",
    conversationTipsAr: [
          "اسأل ما الذي يُحصد قريبًا هذا الأسبوع — الـ제철 (جيتشول، «دورة الموسم») مصدر فخر محلي حقيقي.",
          "إن لم يكن الطبق على القائمة المطبوعة فاسأل عنه رغم ذلك: أفضل صنف موسمي غالبًا غير مدرج.",
    ],
    passportMissionAr: { title: "دع الموسم يطلب", detail: "اسأل 제철 음식이 뭐예요؟ واطلب ما يأتيك في الجواب — ولو لم تعرف نطقه بعد." },
    culturalMeaningAr: "الأكل على الـ제철 (جيتشول) يعامل التقويم بوصفه القائمة الحقيقية: أن تأكل على خطى ما تنتجه الأرض الآن.",
    whenKoreansEatThisAr: "مع كل دورة موسم — الطبق الذي على مائدتك هذا الشهر لن يكون عليها بعد ستة أسابيع.",
    didYouKnowZh: "韩国菜跟着 제철（jecheol）走，也就是\"当季\"。食材进入季节时菜单会悄悄地变，所以本地人问的是\"今天什么好\"，而不是去读菜单。",
    diningTipsZh: [
          "问问什么是当季的——最好的那道菜常常不在印出来的菜单上。",
          "食物里程这么短，你盘子里的蔬菜很可能是一两天前才收的。",
    ],
    whyLocalsLoveItZh: "常客不要菜单：他们问什么当季，因为厨房换得比印刷版跟得上的还快。",
    conversationTipsZh: [
          "问问这周附近收的是什么——제철（jecheol，\"当季\"）是本地实打实的一份骄傲。",
          "一道菜不在印出来的菜单上也照样问：最好的时令那样往往没写。",
    ],
    passportMissionZh: { title: "让季节点菜", detail: "问一句 제철 음식이 뭐예요？然后点回答里的那样——哪怕你还念不出它的名字。" },
    culturalMeaningZh: "按 제철（jecheol）吃，是把日历当成真正的菜单：跟着土地此刻正在产出的东西吃。",
    whenKoreansEatThisZh: "每逢季节转换——这个月在你桌上的那道菜，六周后就不在了。",
    didYouKnowKo: "한국 요리는 제철을 따릅니다. 재료가 철을 맞으면 메뉴가 조용히 바뀌고, 그래서 현지 사람들은 메뉴판을 읽는 대신 \"오늘 뭐가 좋아요?\"라고 묻습니다.",
    diningTipsKo: [
          "제철이 뭔지 물어보세요. 가장 좋은 요리는 인쇄된 메뉴판에 없는 경우가 많습니다.",
          "이동 거리가 이 정도로 짧으면, 접시 위의 채소는 하루 이틀 안에 거둔 것일 가능성이 큽니다."
    ],
    whyLocalsLoveItKo: "단골은 메뉴판을 달라고 하지 않습니다. 뭐가 제철이냐고 묻지요. 주방이 인쇄물보다 먼저 바꿔 버리니까요.",
    conversationTipsKo: [
          "이번 주에 근처에서 뭘 거뒀는지 물어보세요. 제철은 진심 어린 지역의 자부심입니다.",
          "인쇄된 메뉴에 없어도 일단 물어보세요. 가장 좋은 제철 음식은 대개 적혀 있지 않습니다."
    ],
    passportMissionKo: {
          "title": "계절이 주문하게 두기",
          "detail": "\"제철 음식이 뭐예요?\"라고 묻고, 돌아온 대답을 그대로 시켜 보세요. 아직 발음이 안 되더라도요."
    },
    culturalMeaningKo: "제철 식사는 달력을 진짜 메뉴판으로 삼습니다. 지금 이 땅이 내놓는 것에 발을 맞춰 먹는 일이에요.",
    whenKoreansEatThisKo: "철이 바뀔 때마다. 이번 달 상에 오른 요리는 여섯 주 뒤에는 없습니다.",
    didYouKnow:
      "Korean cooking follows 제철 (jecheol) — 'the season's turn.' Menus quietly change as ingredients come into season, which is why locals ask 'what's good today?' instead of reading the menu.",
    diningTips: [
      'Ask what is seasonal — the best dish is often not on the printed menu.',
      'With food mileage this low, the vegetables on your plate were likely harvested within a day or two.',
    ],
    whyLocalsLoveIt:
      "Regulars don't ask for the menu — they ask what's in season, because the kitchen changes it before the printed version catches up.",
    usefulKorean: [
      { ko: '제철 음식이 뭐예요?', ro: 'jecheol eumsigi mwoyeyo?', en: "What's in season right now?", ko_gloss: '제철 음식이 뭔지 묻는 말', es: '¿Qué es de temporada ahora?', fr: "Qu'est-ce qui est de saison en ce moment ?", ar: "ما الذي في موسمه الآن؟", zh: "现在什么是当季的？" },
      { ko: '오늘의 메뉴', ro: 'oneurui menyu', en: "today's menu", ko_gloss: "오늘의 메뉴", es: "el menú de hoy", fr: "le menu du jour", ar: "قائمة اليوم", zh: "今日菜单" },
      { ko: '잘 먹었습니다', ro: 'jal meogeotseumnida', en: 'That was a good meal (said after eating)', ko_gloss: '식사 후에 하는 인사', es: 'He comido muy bien (se dice al terminar)', fr: "C'était un bon repas (se dit après avoir mangé)", ar: "كانت وجبة طيّبة (تُقال بعد الأكل)", zh: "这顿饭很好（吃完说的）" },
    ],
    conversationTips: [
      'Ask what\'s harvested nearby this week — 제철 (jecheol, "the season\'s turn") is a genuine point of local pride.',
      "If a dish isn't on the printed menu, ask anyway — the best seasonal item often isn't listed.",
    ],
    passportMission: { title: 'Let the Season Order', detail: 'Ask 제철 음식이 뭐예요? and order whatever comes back — even if you cannot pronounce it yet.' },
    culturalMeaning: '제철 (jecheol) dining treats the calendar as the real menu — eating in step with what the land is producing right now.',
    whenKoreansEatThis: "Whenever the season turns — the dish on your table this month won't be on it in six weeks.",
  },
};

// Short, scannable label for the category tag on cards. Kept separate from
// the category key itself so the key can stay code-shaped (kebab-case) while
// the label reads naturally.
export const CATEGORY_LABEL = {
  temple: 'Temple Cuisine',
  'korean-chinese': 'Korean-Chinese',
  'vegan-dining': 'Plant-Based',
  'halal-korean': 'Halal Korean',
  'world-halal': 'World Halal',
  'zero-waste': 'Zero-Waste',
  'brunch-bakery': 'Brunch & Bakery',
  'local-seasonal': 'Seasonal',
};

// The same eight labels in Korean. Keyed identically so a category added to
// one map and forgotten in the other renders the English rather than nothing.
export const CATEGORY_LABEL_KO = {
  temple: '사찰음식',
  'korean-chinese': '한국식 중화요리',
  'vegan-dining': '식물성 식당',
  'halal-korean': '할랄 한식',
  'world-halal': '세계 할랄',
  'zero-waste': '제로웨이스트',
  'brunch-bakery': '브런치·베이커리',
  'local-seasonal': '제철',
};

export const CATEGORY_LABEL_ES = {
  temple: 'Cocina de templo',
  'korean-chinese': 'Coreano-chino',
  'vegan-dining': 'Cocina vegetal',
  'halal-korean': 'Coreano halal',
  'world-halal': 'Halal del mundo',
  'zero-waste': 'Sin residuos',
  'brunch-bakery': 'Brunch y panadería',
  'local-seasonal': 'De temporada',
};

// Neighbourhood names as Koreans write them. An address is not prose, so
// this is transliteration rather than translation — and a zone missing from
// here falls back to the romanised form, which is still readable.
export const ZONE_KO = {
  'Jongno, Seoul': '서울 종로',
  'Insadong, Seoul': '서울 인사동',
  'Itaewon, Seoul': '서울 이태원',
  'Myeongdong, Seoul': '서울 명동',
  'Hoehyeon, Seoul': '서울 회현',
  'Bukchon, Seoul': '서울 북촌',
  'Seochon, Seoul': '서울 서촌',
  'Songdo, Incheon': '인천 송도',
  'Guwol-dong, Incheon': '인천 구월동',
  'Cheongna, Incheon': '인천 청라',
  'Yeonsu, Incheon': '인천 연수',
  'Chinatown, Jemulpo-gu, Incheon': '인천 제물포구 차이나타운',
  'Gaehang-ro, Incheon': '인천 개항로',
};

// Neighbourhood names in Spanish. Almost all of them are the same word — a
// district is a proper noun, not a phrase — so only the city part changes.
// Kept as a full map rather than a "translate Seoul, keep the rest" rule,
// because that rule is one exception away from being wrong.
export const ZONE_ES = {
  'Jongno, Seoul': 'Jongno, Seúl',
  'Insadong, Seoul': 'Insadong, Seúl',
  'Itaewon, Seoul': 'Itaewon, Seúl',
  'Myeongdong, Seoul': 'Myeongdong, Seúl',
  'Hoehyeon, Seoul': 'Hoehyeon, Seúl',
  'Bukchon, Seoul': 'Bukchon, Seúl',
  'Seochon, Seoul': 'Seochon, Seúl',
  'Songdo, Incheon': 'Songdo, Incheon',
  'Guwol-dong, Incheon': 'Guwol-dong, Incheon',
  'Cheongna, Incheon': 'Cheongna, Incheon',
  'Yeonsu, Incheon': 'Yeonsu, Incheon',
  'Chinatown, Jemulpo-gu, Incheon': 'Barrio chino, Jemulpo-gu, Incheon',
  'Gaehang-ro, Incheon': 'Gaehang-ro, Incheon',
};

// The same categories in French.
export const CATEGORY_LABEL_FR = {
  temple: "Cuisine de temple",
  "korean-chinese": "Coréen-chinois",
  "vegan-dining": "Cuisine végétale",
  "halal-korean": "Coréen halal",
  "world-halal": "Halal du monde",
  "zero-waste": "Zéro déchet",
  "brunch-bakery": "Brunch et boulangerie",
  "local-seasonal": "De saison",
};

// Neighbourhood names in French. Same rule as the Spanish map: a district is
// a proper noun, so only the city part moves.
export const ZONE_FR = {
  "Jongno, Seoul": "Jongno, Séoul",
  "Insadong, Seoul": "Insadong, Séoul",
  "Itaewon, Seoul": "Itaewon, Séoul",
  "Myeongdong, Seoul": "Myeongdong, Séoul",
  "Hoehyeon, Seoul": "Hoehyeon, Séoul",
  "Bukchon, Seoul": "Bukchon, Séoul",
  "Seochon, Seoul": "Seochon, Séoul",
  "Songdo, Incheon": "Songdo, Incheon",
  "Guwol-dong, Incheon": "Guwol-dong, Incheon",
  "Cheongna, Incheon": "Cheongna, Incheon",
  "Yeonsu, Incheon": "Yeonsu, Incheon",
  "Chinatown, Jemulpo-gu, Incheon": "Quartier chinois, Jemulpo-gu, Incheon",
  "Gaehang-ro, Incheon": "Gaehang-ro, Incheon",
};

// The same categories in Arabic.
export const CATEGORY_LABEL_AR = {
  temple: "مطبخ المعبد",
  "korean-chinese": "كوري صيني",
  "vegan-dining": "مطبخ نباتي",
  "halal-korean": "كوري حلال",
  "world-halal": "حلال من العالم",
  "zero-waste": "بلا نفايات",
  "brunch-bakery": "فطور ومخبوزات",
  "local-seasonal": "من الموسم",
};

// Neighbourhood names in Arabic. A district is a proper noun, so these are
// transliterations rather than translations — the one word that changes is
// the city, and the Chinatown, which is a description in every language.
export const ZONE_AR = {
  "Jongno, Seoul": "جونغنو، سول",
  "Insadong, Seoul": "إنسا-دونغ، سول",
  "Itaewon, Seoul": "إيتايوون، سول",
  "Myeongdong, Seoul": "ميونغ-دونغ، سول",
  "Hoehyeon, Seoul": "هويهيون، سول",
  "Bukchon, Seoul": "بوكتشون، سول",
  "Seochon, Seoul": "سوتشون، سول",
  "Songdo, Incheon": "سونغدو، إنتشون",
  "Guwol-dong, Incheon": "غوول-دونغ، إنتشون",
  "Cheongna, Incheon": "تشونغنا، إنتشون",
  "Yeonsu, Incheon": "يونسو، إنتشون",
  "Chinatown, Jemulpo-gu, Incheon": "الحي الصيني، جيمولبو-غو، إنتشون",
  "Gaehang-ro, Incheon": "غيهانغ-رو، إنتشون",
};

// The same categories in Simplified Chinese.
export const CATEGORY_LABEL_ZH = {
  temple: "寺院饮食",
  "korean-chinese": "韩式中餐",
  "vegan-dining": "植物性餐厅",
  "halal-korean": "清真韩餐",
  "world-halal": "各国清真",
  "zero-waste": "零废弃",
  "brunch-bakery": "早午饭与烘焙",
  "local-seasonal": "当季",
};

// Neighbourhood names in Chinese. Korean place names have their own Hanja
// readings, so these are the established Chinese forms rather than a
// transliteration of the romanisation — 종로 is 钟路, not 琼路.
export const ZONE_ZH = {
  "Jongno, Seoul": "首尔钟路",
  "Insadong, Seoul": "首尔仁寺洞",
  "Itaewon, Seoul": "首尔梨泰院",
  "Myeongdong, Seoul": "首尔明洞",
  "Hoehyeon, Seoul": "首尔会贤",
  "Bukchon, Seoul": "首尔北村",
  "Seochon, Seoul": "首尔西村",
  "Songdo, Incheon": "仁川松岛",
  "Guwol-dong, Incheon": "仁川九月洞",
  "Cheongna, Incheon": "仁川青萝",
  "Yeonsu, Incheon": "仁川延寿",
  "Chinatown, Jemulpo-gu, Incheon": "仁川济物浦区中华街",
  "Gaehang-ro, Incheon": "仁川开港路",
};

export function getCulture(place) {
  const base = cultureByCategory[place.category] ?? cultureByCategory['local-seasonal'];
  return {
    didYouKnow: place.didYouKnow ?? base.didYouKnow,
    diningTips: place.diningTips ?? base.diningTips,
    whyLocalsLoveIt: place.whyLocalsLoveIt ?? base.whyLocalsLoveIt,
    usefulKorean: place.usefulKorean ?? base.usefulKorean,
    conversationTips: place.conversationTips ?? base.conversationTips,
    passportMission: place.passportMission ?? base.passportMission,
    culturalMeaning: place.culturalMeaning ?? base.culturalMeaning,
    whenKoreansEatThis: place.whenKoreansEatThis ?? base.whenKoreansEatThis,

    // The Korean twin of each field, carried through the same override chain.
    // A restaurant that overrides `didYouKnow` with its own English line and
    // writes no `didYouKnowKo` gets the category's Korean, which would be the
    // wrong text — so the override is read as a pair: if the place overrode
    // the English, its Korean is whatever the place itself supplies, and
    // nothing is inherited to sit under a sentence it does not translate.
    didYouKnowKo: place.didYouKnow ? place.didYouKnowKo : base.didYouKnowKo,
    diningTipsKo: place.diningTips ? place.diningTipsKo : base.diningTipsKo,
    whyLocalsLoveItKo: place.whyLocalsLoveIt ? place.whyLocalsLoveItKo : base.whyLocalsLoveItKo,
    usefulKoreanKo: place.usefulKorean ? place.usefulKoreanKo : base.usefulKoreanKo,
    conversationTipsKo: place.conversationTips ? place.conversationTipsKo : base.conversationTipsKo,
    passportMissionKo: place.passportMission ? place.passportMissionKo : base.passportMissionKo,
    culturalMeaningKo: place.culturalMeaning ? place.culturalMeaningKo : base.culturalMeaningKo,
    whenKoreansEatThisKo: place.whenKoreansEatThis ? place.whenKoreansEatThisKo : base.whenKoreansEatThisKo,

    // Spanish, through the same override chain and for the same reason.
    didYouKnowEs: place.didYouKnow ? place.didYouKnowEs : base.didYouKnowEs,
    diningTipsEs: place.diningTips ? place.diningTipsEs : base.diningTipsEs,
    whyLocalsLoveItEs: place.whyLocalsLoveIt ? place.whyLocalsLoveItEs : base.whyLocalsLoveItEs,
    conversationTipsEs: place.conversationTips ? place.conversationTipsEs : base.conversationTipsEs,
    passportMissionEs: place.passportMission ? place.passportMissionEs : base.passportMissionEs,
    culturalMeaningEs: place.culturalMeaning ? place.culturalMeaningEs : base.culturalMeaningEs,
    whenKoreansEatThisEs: place.whenKoreansEatThis ? place.whenKoreansEatThisEs : base.whenKoreansEatThisEs,

    // French, through the same override chain and for the same reason.
    didYouKnowFr: place.didYouKnow ? place.didYouKnowFr : base.didYouKnowFr,
    diningTipsFr: place.diningTips ? place.diningTipsFr : base.diningTipsFr,
    whyLocalsLoveItFr: place.whyLocalsLoveIt ? place.whyLocalsLoveItFr : base.whyLocalsLoveItFr,
    conversationTipsFr: place.conversationTips ? place.conversationTipsFr : base.conversationTipsFr,
    passportMissionFr: place.passportMission ? place.passportMissionFr : base.passportMissionFr,
    culturalMeaningFr: place.culturalMeaning ? place.culturalMeaningFr : base.culturalMeaningFr,
    whenKoreansEatThisFr: place.whenKoreansEatThis ? place.whenKoreansEatThisFr : base.whenKoreansEatThisFr,

    // Arabic, through the same override chain and for the same reason.
    didYouKnowAr: place.didYouKnow ? place.didYouKnowAr : base.didYouKnowAr,
    diningTipsAr: place.diningTips ? place.diningTipsAr : base.diningTipsAr,
    whyLocalsLoveItAr: place.whyLocalsLoveIt ? place.whyLocalsLoveItAr : base.whyLocalsLoveItAr,
    conversationTipsAr: place.conversationTips ? place.conversationTipsAr : base.conversationTipsAr,
    passportMissionAr: place.passportMission ? place.passportMissionAr : base.passportMissionAr,
    culturalMeaningAr: place.culturalMeaning ? place.culturalMeaningAr : base.culturalMeaningAr,
    whenKoreansEatThisAr: place.whenKoreansEatThis ? place.whenKoreansEatThisAr : base.whenKoreansEatThisAr,

    // Chinese, through the same override chain and for the same reason.
    didYouKnowZh: place.didYouKnow ? place.didYouKnowZh : base.didYouKnowZh,
    diningTipsZh: place.diningTips ? place.diningTipsZh : base.diningTipsZh,
    whyLocalsLoveItZh: place.whyLocalsLoveIt ? place.whyLocalsLoveItZh : base.whyLocalsLoveItZh,
    conversationTipsZh: place.conversationTips ? place.conversationTipsZh : base.conversationTipsZh,
    passportMissionZh: place.passportMission ? place.passportMissionZh : base.passportMissionZh,
    culturalMeaningZh: place.culturalMeaning ? place.culturalMeaningZh : base.culturalMeaningZh,
    whenKoreansEatThisZh: place.whenKoreansEatThis ? place.whenKoreansEatThisZh : base.whenKoreansEatThisZh,
  };
}
