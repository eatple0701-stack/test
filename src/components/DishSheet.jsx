  ...
  return (
    <div className="dish-sheet sheet-page" role="dialog" aria-label={`${menu.name}`}>
      ...
      <div className="dish-sheet__body">
        <p className="dish-sheet__gloss">{say(menu.gloss, menu.glossKo, menu.glossEs, menu.glossFr, menu.glossAr)}</p>
        ...
        <section className="dish-sheet__block">
          <h2 className="dish-sheet__label">{say('Why it is shared', '왜 나눠 먹나', 'Por qué se comparte', 'Pourquoi cela se partage', 'لماذا يُشارَك')}</h2>
          <p>{say(menu.whyShared, menu.whySharedKo, menu.whySharedEs, menu.whySharedFr, menu.whySharedAr)}</p>
        </section>

        <section className="dish-sheet__block">
          <h2 className="dish-sheet__label">{say('What happens at the table', '식탁에서 벌어지는 일', 'Qué pasa en la mesa', 'Ce qui se passe à table', 'ما الذي يحدث على المائدة')}</h2>
          <p>{say(menu.howItWorks, menu.howItWorksKo, menu.howItWorksEs, menu.howItWorksFr, menu.howItWorksAr)}</p>
        ...
        {menu.culture && (
          <section className="dish-sheet__block">
            <h2 className="dish-sheet__label">{say('Why it is eaten together', '왜 함께 먹나', 'Por qué se come en compañía', 'Pourquoi on le mange ensemble', 'لماذا يُؤكل مع الناس')}</h2>
            <p>{say(menu.culture, menu.cultureKo, menu.cultureEs, menu.cultureFr, menu.cultureAr)}</p>
          </section>
        )}
        ...
        <button className="dish-sheet__cta" translate="no" onClick={() => onOpenTable?.(menu.id)}>
          {say('이 요리로 상 차리기 · Open a table for this', '이 요리로 상 차리기', 'Abrir una mesa con este plato', 'Ouvrir une table pour ce plat', 'افتح مائدة لهذا الطبق')}
        </button>
      </div>
    </div>
  );
