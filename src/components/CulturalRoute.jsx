    ...
    <ol className="route-list">
      <li className="route-step route-step--current">
        ...
        <div className="route-step__body">
          ...
          <span className="route-step__note">{say('You are here', '지금 여기', 'Estás aquí', 'Vous êtes ici', 'أنت هنا', '你在这儿', '現在地')}</span>
        </div>
      </li>
      {steps.map((step, i) => (
          ...
          <button className="route-step__body route-step__body--tappable" onClick={() => handle(step)}>
            <span className="route-step__label">
              {step.place
                ...
                : say(step.label, step.labelKo, step.labelEs, step.labelFr, step.labelAr, step.labelZh, step.labelJa)}
              <ChevronRightIcon size={14} />
            </span>
            <span className="route-step__note">{say(step.note, step.noteKo, step.noteEs, step.noteFr, step.noteAr, step.noteZh, step.noteJa)}</span>
          </button>
        </li>
      ))}
    </ol>
