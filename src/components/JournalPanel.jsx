...
export default function JournalPanel({
  ...
  return (
    <section className="journal-panel" aria-label={say('Journal', '여권', 'Pasaporte', 'Passeport')}>
      <header className="screen-head screen-head--dark">
        <div className="screen-head__row">
          <div>
            <span className="screen-head__kr">여권</span>
            <h1 className="screen-head__title">
              {say('What this trip has been so far.', '이번 여행이 지금까지 어땠는지.', 'Lo que ha sido este viaje hasta ahora.', "Ce qu'a été ce voyage jusqu'ici.")}
            </h1>
          </div>
          {onOpenSummary && (
            <button className="screen-head__link" onClick={onOpenSummary}>
              {say('Share', '공유', 'Compartir', 'Partager')}
            </button>
          )}
        </div>
        ...
        <p className="screen-head__sub">
          {recordCount > 0
            ? say(`${recordCount} moment${recordCount === 1 ? '' : 's'} recorded.`,
              ...
              `${recordCount} ${recordCount === 1 ? 'momento registrado' : 'momentos registrados'}.`, `${recordCount} ${recordCount === 1 ? 'moment enregistré' : 'moments enregistrés'}.`)
            : upcomingTables.length > 0
              ...
              `Rien d'enregistré pour l'instant — ${upcomingTables.length === 1
                  ...
                  : `${upcomingTables.length} tables réservées. Elles arriveront ici ensuite.`}`)
              : say('Nothing recorded yet — whatever you do lands here.',
                '아직 기록이 없어요 — 무엇을 하시든 여기에 남습니다.', 'Todavía no hay nada registrado — lo que hagas aparecerá aquí.', "Rien d'enregistré pour l'instant — ce que vous ferez apparaîtra ici.")}
        </p>
      </header>
        ...
        <div className="member-gate member-gate--inline">
          <h3 className="member-gate__title">
            ...
            <span className="member-gate__title-en">
              {say(gateText('passport').titleEn, null, gateText('passport').titleEs, gateText('passport').titleFr)}
            </span>
          </h3>
          <p className="member-gate__body">{say(gateText('passport').body, gateText('passport').bodyKo, gateText('passport').bodyEs, gateText('passport').bodyFr)}</p>
          <button className="auth-primary" translate="no" onClick={() => onRequireAuth?.('passport')}>
            {say(gateText('passport').cta, null, gateText('passport').ctaEs, gateText('passport').ctaFr)}
          </button>
        </div>
      )}
      ...
      {upcomingTables.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('Coming up', '다가오는 밥상', 'Próximamente', 'À venir')}</h3>
                ...
                <div key={t.id} className="upcoming-row">
                  ...
                  <div className="upcoming-row__body">
                    <span className="upcoming-row__dish">
                      {menu.name}
                      {t.hosted && <span className="upcoming-row__badge">{say('you host', '내가 호스트', 'eres anfitrión', 'vous êtes hôte')}</span>}
                      ...
                      {t.myRequest && isPending(t.myRequest) && !hasLapsed(t.myRequest, t) && (
                        <span className="upcoming-row__pending">{say('waiting on the host', '호스트 응답 대기', 'esperando al anfitrión', "en attente de l'hôte")}</span>
                      )}
                      {t.myRequest && hasLapsed(t.myRequest, t) && (
                        <span className="upcoming-row__lapsed">{say('no answer — seat released', '응답 없음 — 자리 풀림', 'sin respuesta: sitio liberado', 'sans réponse : place libérée')}</span>
                      )}
                      ...
                      {isCancelled(t) && (
                        <span className="upcoming-row__cancelled">{say('called off — do not go', '취소됨 — 가지 마세요', 'cancelada: no vayas', "annulée : n'y allez pas")}</span>
                      )}
                    </span>
      ...
      {days.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('Your record', '내 기록', 'Tu registro', 'Votre registre')}</h3>
      ...
      <div className="journal-section passport-summary">
        <div className="journal-section-header">
          <h3>{say('This trip so far', '이번 여행, 지금까지', 'Este viaje hasta ahora', "Ce voyage jusqu'ici")}</h3>
        </div>
        <div className="passport-stats">
          <div className="stat-box">
            <span className="stat-num">{eaten.length}</span>
            <span className="stat-label">{say('밥상 · tables', '밥상', 'mesas', 'tables')}</span>
          </div>
          <div className="stat-box">
            <span className="stat-num">{metPeople.length}</span>
            <span className="stat-label">{say('사람 · met', '사람', 'personas', 'rencontres')}</span>
          </div>
          <div className="stat-box">
            <span className="stat-num">{journey.experienceCount}</span>
            <span className="stat-label">{say('문화 · done', '문화', 'culturas', 'cultures')}</span>
          </div>
          <div className="stat-box">
            <span className="stat-num">{journey.foodCount}</span>
            <span className="stat-label">{say('장소 · visited', '장소', 'sitios', 'lieux')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="journal-section">
        <div className="journal-section-header">
          <h3>{say('Worth doing', '해볼 만한 것', 'Merece la pena', 'À faire')}</h3>
          ...
          {goals.map(g => (
            <li key={g.id} className={`goal${g.done ? ' is-done' : ''}`}>
              ...
              <span className="goal__body">
                <span className="goal__name">{say(g.name, g.nameKo, g.nameEs, g.nameFr)}</span>
                <span className="goal__hint">
                  {g.done
                    ? say('Done.', '완료.', 'Hecho.', 'Fait.')
                    : `${say(g.hint, g.hintKo, g.hintEs, g.hintFr)} ${g.current}/${g.target}`}
                </span>
              </span>
            </li>
          ))}
      ...
      {metPeople.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('People Met', '만난 사람', 'Personas conocidas', 'Personnes rencontrées')}</h3>
          </div>
      ...
      {visitedList.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('Visited Places', '가본 곳', 'Sitios visitados', 'Lieux visités')}</h3>
          </div>
          <div className="journal-grid">
      ...
      {walkedThemes.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('Cultures you walked', '걸어본 문화', 'Culturas recorridas', 'Cultures parcourues')}</h3>
      ...
      {blocks.length > 0 && (
        <div className="journal-section">
          <div className="journal-section-header">
            <h3>{say('Blocked', '차단됨', 'Bloqueado', 'Bloqué')}</h3>
            ...
            {blocks.map(b => (
              <li key={b.blockedId} className="blocked-row">
                ...
                <button className="blocked-row__undo" onClick={() => unblock(b.blockedId)}>
                  {say('Unblock', '차단 해제', 'Desbloquear', 'Débloquer')}
                </button>
              </li>
            ))}
          </ul>
