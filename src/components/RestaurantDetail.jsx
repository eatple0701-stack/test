...
import {
  dietaryBadges, isKnown, needsCheck, trustBadge, dietaryConfidence, CONFIDENCE, isQuarantined, TRUST_LABEL_KO, TRUST_LABEL_ES, TRUST_LABEL_FR, TRUST_LABEL_AR, TRUST_LABEL_ZH } from '../data/verification';
...
import { ZONE_KO, ZONE_ES, ZONE_FR, ZONE_AR, ZONE_ZH } from '../data/culture';

const TRAIT_META = {
  'Mild Taste': { Icon: MildIcon, label: 'Mild taste', labelKo: '맵지 않음', labelEs: 'Suave', labelFr: 'Peu relevé', labelAr: 'غير حارّ', labelZh: '不辣' },
  'Fermented': { Icon: FermentIcon, label: 'Fermented', labelKo: '발효', labelEs: 'Fermentado', labelFr: 'Fermenté', labelAr: 'مخمّر', labelZh: '发酵' },
  'Zero-waste': { Icon: RecycleIcon, label: 'Zero waste', labelKo: '제로웨이스트', labelEs: 'Sin residuos', labelFr: 'Zéro déchet', labelAr: 'بلا نفايات', labelZh: '零废弃' },
  'Local Sourcing': { Icon: SproutIcon, label: 'Locally sourced', labelKo: '지역 재료', labelEs: 'Producto local', labelFr: 'Produit local', labelAr: 'منتج محلي', labelZh: '本地食材' },
};
...
function SectionHead({ Icon, title, ko, es, fr, ar, zh, kr }) {
  const say = useText();
  const heading = say(title, ko, es, fr, ar, zh);
...
function Trust({ fact }) {
  ...
  return <span className={`trust trust--${tone}`} title={detail}>{say(label, TRUST_LABEL_KO[label], TRUST_LABEL_ES[label], TRUST_LABEL_FR[label], TRUST_LABEL_AR[label], TRUST_LABEL_ZH[label])}</span>;
}
  ...
  return (
    <>
      ...
      <div className="detail-sheet" role="dialog" aria-modal="true" aria-label={name} ref={sheetRef} tabIndex={-1}>
        <button className="detail-close" aria-label={say('Close', '닫기', 'Cerrar', 'Fermer', 'إغلاق', '关闭')} onClick={onClose}>
          <XIcon size={18} />
        </button>
          ...
          <div className="detail-content">
            ...
            <header className="detail-header">
              ...
              <p className="detail-meta">
                {say(restaurant.zone, ZONE_KO[restaurant.zone], ZONE_ES[restaurant.zone], ZONE_FR[restaurant.zone], ZONE_AR[restaurant.zone], ZONE_ZH[restaurant.zone])}
            ...
            {/* 3. Diet Tags */}
            {facts.length > 0 && (
              <ul className="fact-row" aria-label={say('Dietary and dining facts', '식단과 식사 정보', 'Datos de dieta y de mesa', 'Informations sur le régime et sur la table', 'معلومات عن النظام الغذائي والمائدة', '饮食与餐桌信息')}>
                {facts.map(({ Icon, label, labelKo, labelEs, labelFr, labelAr, labelZh, fact: f }) => (
                  <li key={label} className="fact">
                    <Icon size={16} aria-hidden="true" /> {say(label, labelKo, labelEs, labelFr, labelAr, labelZh)}
                    {f && <Trust fact={f} />}
                  </li>
                ))}
              </ul>
            ...
            <div className="diet-note">
              <p>
                <strong>{say(caveat.title, caveat.titleKo, caveat.titleEs, caveat.titleFr, caveat.titleAr, caveat.titleZh)}</strong>
                {' '}{say(caveat.body, caveat.bodyKo, caveat.bodyEs, caveat.bodyFr, caveat.bodyAr, caveat.bodyZh)}
              </p>
              {certClaim && (
                <p className="diet-note__cert">
                  {say(`Certification claimed: ${certClaim.body} — we have not sighted the certificate.`,
                    ...
                    `Certificación declarada: ${certClaim.body} — no hemos visto el certificado.`, `Certification déclarée : ${certClaim.body} — nous n'avons pas vu le certificat.`, `الاعتماد مُعلَن: ${certClaim.body} — ولم نرَ الشهادة.`, `声称已认证：${certClaim.body} — 我们没有见到证书。`)}
                </p>
              )}
            </div>
            ...
            <div className="place-actions">
              <div className="place-actions__row">
                ...
                >
                  ...
                  {isBookmarked
                    ? say('저장됨 · Saved', '저장됨', 'Guardado', 'Enregistré', 'محفوظ', '已保存')
                    : say('저장하기 · Save this place', '저장하기', 'Guardar este sitio', 'Enregistrer cette adresse', 'احفظ هذا المكان', '保存这个地方')}
                </button>
              </div>
              {isBookmarked && (
                <button className="saved-receipt" onClick={() => onNavigate?.('journal')}>
                  {say('패스포트 › 저장한 곳에 있어요 · Find it under Saved places in your Passport', '패스포트 › 저장한 곳에 있어요', 'Lo tienes en Sitios guardados, en tu Pasaporte', 'Vous le retrouverez dans Lieux enregistrés, dans votre Passeport', 'ستجده في الأماكن المحفوظة، في جواز سفرك', '在护照的「保存的地点」里能找到')}
                </button>
              )}
              ...
              {mapLinks.length > 0 && (
                <div className="map-links">
                  <p className="map-links__note">
                    <strong>{MAP_LINKS_NOTE.kr}</strong> {say(MAP_LINKS_NOTE.en, null, MAP_LINKS_NOTE.es, MAP_LINKS_NOTE.fr, MAP_LINKS_NOTE.ar, MAP_LINKS_NOTE.zh)}
                  </p>
              ...
              {transit && (
                <p className="place-actions__transit">
                  ...
                  <span className="place-actions__transit-en">{say(transit.en, null, transit.es, transit.fr, transit.ar, transit.zh)}</span>
                </p>
              )}
            </div>
            ...
            <section className="detail-section">
              <SectionHead Icon={ClockIcon} title="Quick Info" ko="기본 정보" es="Información básica" fr="Informations pratiques" />
                    ...
                    <span>
                      ...
                      {today && (
                        <span className="practical-muted">
                          ({say('today', '오늘', 'hoy', "aujourd'hui", 'اليوم', '今天')} {today})
                        </span>
                      )}
                    </span>
                  ) : (
                    <span className="practical-muted">
                      {say('Opening hours unknown — check before you go', '영업시간을 알 수 없습니다 — 가시기 전에 확인하세요', 'Horario desconocido: compruébalo antes de ir', 'Horaires inconnus : vérifiez avant de vous déplacer', 'ساعات العمل غير معروفة: تحقّق قبل أن تذهب', '营业时间不详：去之前先确认')}
                    </span>
                  )}
                </div>
                  ...
                  <div className="practical-row">
                    ...
                    <span>
                      ...
                      {restaurant.transit.value.exit && (
                        <span>
                          {say(', exit ', ', ', ', salida ', ', sortie ', '، مخرج ', '，出口 ')}{restaurant.transit.value.exit}
                          {say('', '번 출구', '', '', '', '')}
                        </span>
                      )}
                      <span>
                        ...
                        {say(`${restaurant.transit.value.walkingMinutes} min walk`,
                          ...
                          `${restaurant.transit.value.walkingMinutes} min a pie`, `${restaurant.transit.value.walkingMinutes} min à pied`, `${restaurant.transit.value.walkingMinutes} دقائق سيرًا`, `步行 ${restaurant.transit.value.walkingMinutes} 分钟`)}
                      </span>
                    </span>
                  </div>
                )}
                  ...
                  <div className="practical-row">
                    ...
                    <span className="practical-links">
                      {isKnown(restaurant.officialUrl) && (
                        <a className="practical-link" href={restaurant.officialUrl.value} target="_blank" rel="noreferrer noopener">{say('Website', '웹사이트', 'Sitio web', 'Site web', 'الموقع الإلكتروني', '官网')}</a>
                      )}
            ...
            {isKnown(restaurant.menus) && (
              <section className="detail-section">
                <SectionHead Icon={MenuIcon} title="Signature Menu" ko="대표 메뉴" es="Platos estrella" fr="Les plats signature" />
                <div className="menu-rows">
                  {restaurant.menus.value.map(m => (
                    <div key={m.name} className="menu-row">
                      <span>{say(m.name, m.nameKo, m.nameEs, m.nameFr, m.nameAr, m.nameZh)}</span>
                ...
                {needsCheck(restaurant.menus) && (
                  <p className="section-note">
                    {say('Dishes and prices are unverified and may have changed.',
                      '메뉴와 가격은 확인되지 않았고 바뀌었을 수 있습니다.', 'Los platos y precios no están verificados y pueden haber cambiado.', 'Les plats et les prix ne sont pas vérifiés et ont pu changer.', 'الأطباق والأسعار غير موثّقة وقد تكون تغيّرت.', '菜品和价格未经确认，可能已经变了。')}
                  </p>
                )}
              </section>
            )}
            ...
            <section className="detail-section">
              <SectionHead Icon={SparkleIcon} title="Why Locals Love This" ko="현지인이 좋아하는 이유" es="Por qué gusta a los locales" fr="Pourquoi les gens d'ici y tiennent" />
              <p className="detail-body">{say(culture.whyLocalsLoveIt, culture.whyLocalsLoveItKo, culture.whyLocalsLoveItEs, culture.whyLocalsLoveItFr, culture.whyLocalsLoveItAr, culture.whyLocalsLoveItZh)}</p>
            </section>

            {/* Local Tips — what someone who eats here would tell you */}
            <section className="detail-section">
              <SectionHead Icon={SparkleIcon} title="Local Tips" ko="현지 팁" es="Consejos locales" fr="Conseils sur place" kr="현지 팁" />
              <div className="tip-cards">
                {tipsFor(restaurant).map(t => (
                  <div key={t.tag} className="tip-card">
                    <span className="tip-card__tag">{say(t.tag, t.tagKo, t.tagEs, t.tagFr, t.tagAr, t.tagZh)}</span>
                    <span className="tip-card__detail">{say(t.detail, t.detailKo, t.detailEs, t.detailFr, t.detailAr, t.detailZh)}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="detail-hook">
              <p className="detail-hook__label">{say("Why it's special", '왜 특별한가', 'Por qué es especial', "Pourquoi c'est particulier", 'ما الذي يميّزه', '它特别在哪儿')}</p>
              <p className="detail-hook__quote">&ldquo;{say(restaurant.vibe, restaurant.vibeKo, restaurant.vibeEs, restaurant.vibeFr, restaurant.vibeAr, restaurant.vibeZh)}&rdquo;</p>
            </section>

            <section className="detail-section" ref={storyRef}>
              <SectionHead Icon={BookIcon} title="Food Story" ko="음식 이야기" es="La historia del plato" fr="L'histoire du plat" kr="이야기" />
              <div className="story-grid">
                <div className="story-mini-card">
                  <p className="story-mini-card__label">{say('📜 Origin', '📜 유래', '📜 Origen', '📜 Origine', '📜 الأصل', '📜 由来')}</p>
                  <p>{say(restaurant.story, restaurant.storyKo, restaurant.storyEs, restaurant.storyFr, restaurant.storyAr, restaurant.storyZh)}</p>
                ...
                <div className="story-mini-card">
                  <p className="story-mini-card__label">{say('🌏 Cultural Meaning', '🌏 문화적 의미', '🌏 Significado cultural', '🌏 Signification culturelle', '🌏 المعنى الثقافي', '🌏 文化含义')}</p>
                  <p>{say(culture.culturalMeaning, culture.culturalMeaningKo, culture.culturalMeaningEs, culture.culturalMeaningFr, culture.culturalMeaningAr, culture.culturalMeaningZh)}</p>
                </div>
                <div className="story-mini-card">
                  <p className="story-mini-card__label">{say('🍽 When Koreans Eat This', '🍽 한국 사람들은 언제 먹나', '🍽 Cuándo lo comen los coreanos', '🍽 Quand les Coréens le mangent', '🍽 متى يأكله الكوريون', '🍽 韩国人什么时候吃')}</p>
                  <p>{say(culture.whenKoreansEatThis, culture.whenKoreansEatThisKo, culture.whenKoreansEatThisEs, culture.whenKoreansEatThisFr, culture.whenKoreansEatThisAr, culture.whenKoreansEatThisZh)}</p>
                </div>
                <div className="story-mini-card">
                  <p className="story-mini-card__label">{say('✨ Fun Fact', '✨ 알아두면 좋은 것', '✨ Un dato curioso', '✨ Un détail curieux', '✨ معلومة طريفة', '✨ 一个小知识')}</p>
                  <p>{say(culture.didYouKnow, culture.didYouKnowKo, culture.didYouKnowEs, culture.didYouKnowFr, culture.didYouKnowAr, culture.didYouKnowZh)}</p>
                </div>
              </div>
            </section>
            ...
            {/* Dining Etiquette */}
            <section className="detail-section">
              <SectionHead Icon={SparkleIcon} title="Dining Etiquette" ko="식사 예절" es="Etiqueta en la mesa" fr="À table, les usages" />
              <ul className="tips-list">
                {(say(culture.diningTips, culture.diningTipsKo, culture.diningTipsEs, culture.diningTipsFr, culture.diningTipsAr, culture.diningTipsZh)).map(tip => (
            ...
            {culture.usefulKorean?.length > 0 && (
              <section className="detail-section">
                <SectionHead Icon={BookIcon} title="Useful Korean" ko="유용한 한국어" es="Coreano útil" fr="Du coréen utile" kr="유용한 한국어" />
                <div className="phrase-list">
                  {culture.usefulKorean.map(p => (
                    <div key={p.ko} className="phrase-row">
                      ...
                      <span className="phrase-en">{say(p.en, p.ko_gloss, p.es, p.fr, p.ar, p.zh)}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {culture.conversationTips?.length > 0 && (
              <section className="detail-section">
                <SectionHead Icon={SparkleIcon} title="Conversation Tips" ko="대화 거리" es="De qué hablar" fr="De quoi parler" />
                <ul className="tips-list">
                  {(say(culture.conversationTips, culture.conversationTipsKo, culture.conversationTipsEs, culture.conversationTipsFr, culture.conversationTipsAr, culture.conversationTipsZh)).map(tip => (
                    <li key={tip} className="tip">
                      <span className="tip__dot" aria-hidden="true" />
                      <span>{tip}</span>
                    </li>
            ...
            <section className="detail-section detail-section--route">
              <SectionHead Icon={CompassIcon} title="Continue Your Journey" ko="여정 잇기" es="Sigue tu recorrido" fr="Poursuivre le parcours" kr="여정 잇기" />
              <p className="section-note">{say('Where this meal leads next.', '이 한 끼가 다음으로 이어지는 곳.', 'Adónde lleva esta comida después.', 'Où mène ce repas ensuite.', 'إلى أين تقود هذه الوجبة بعد ذلك.', '这顿饭之后通向哪儿。')}</p>
              <CulturalRoute
                place={restaurant}
            ...
            {/* Nearby Experiences */}
            <section className="detail-section">
              <SectionHead Icon={MapPinIcon} title="Nearby Experiences" ko="근처에서 해볼 것" es="Cerca de aquí" fr="À faire dans le coin" />
              <p className="detail-body">
                {zoneInfo
                  ? say(zoneInfo.blurb, zoneInfo.blurbKo, zoneInfo.blurbEs, zoneInfo.blurbFr, zoneInfo.blurbAr, zoneInfo.blurbZh)
                  : say(`More of ${restaurant.zone.split(',')[0]} is waiting just outside.`,
                    `${restaurant.zone.split(',')[0]}은(는) 문 밖에 더 있습니다.`,
                    `Hay más de ${restaurant.zone.split(',')[0]} justo ahí fuera.`, `Il y a plus de ${restaurant.zone.split(',')[0]} juste dehors.`, `هناك المزيد من ${restaurant.zone.split(',')[0]} خارج الباب مباشرة.`, `${restaurant.zone.split(',')[0]}还有更多，就在门外。`)}
              </p>
              <div className="cta-stack">
                {onExploreZone && (
                  <button className="btn-primary" onClick={() => onExploreZone(restaurant.zone)}>
                    {say('Explore Nearby', '근처 둘러보기', 'Explorar la zona', 'Explorer les environs', 'استكشف ما حولك', '逛逛附近')} <ChevronRightIcon size={16} />
                  </button>
                )}
              </div>
            </section>
            ...
            {relatedPlaces.length > 0 && (
              <section className="detail-section">
                <SectionHead Icon={BowlIcon} title="Related Foods" ko="관련 음식" es="Platos relacionados" fr="Plats voisins" />
            ...
            {culture.passportMission && (
              <section className="detail-section">
                <SectionHead Icon={SparkleIcon} title="Passport Mission" ko="여권 미션" es="Misión de Pasaporte" fr="Mission de Passeport" kr="여권 미션" />
                <p className="detail-body">
                  <strong>{say(culture.passportMission, culture.passportMissionKo, culture.passportMissionEs, culture.passportMissionFr, culture.passportMissionAr, culture.passportMissionZh).title}</strong> — {say(culture.passportMission, culture.passportMissionKo, culture.passportMissionEs, culture.passportMissionFr, culture.passportMissionAr, culture.passportMissionZh).detail}
                </p>
                <div className={`mission-status${isVisited ? ' mission-status--done' : ''}`}>
                  {isVisited ? (
                    <><CheckIcon size={16} /> {say('Mission complete — logged in your Passport', '미션 완료 — 여권에 기록되었습니다', 'Misión cumplida — registrada en tu Pasaporte', 'Mission accomplie — inscrite dans votre Passeport', 'أُنجزت المهمّة — سُجّلت في جواز سفرك', '任务完成 — 已记进你的护照')}</>
                  ) : (
                    say('Mark this place visited to complete the mission', '이곳을 가봤다고 표시하면 미션이 완료됩니다', 'Marca este sitio como visitado para completar la misión', 'Marquez cette adresse comme visitée pour terminer la mission', 'ضع علامة زُرت هذا المكان لإتمام المهمّة', '把这个地方标记为去过，就完成这个任务')
                  )}
                </div>
              </section>
            )}
            ...
            <section className="detail-section">
              <SectionHead Icon={CompassIcon} title="Location & Directions" ko="위치와 길찾기" es="Ubicación y cómo llegar" fr="Où c'est et comment y aller" />
              ...
              <div className="practical-row">
                ...
                <span>
                  ...
                  {restaurant.address.precision === 'area' && (
                    <span className="practical-muted"> {say('— area only', '— 구역 단위', '— solo la zona', '— le quartier seulement', '— الحيّ فقط', '— 只到街区')}</span>
                  )}
                </span>
                <button className="practical-copy" onClick={handleCopy}>
                  {copied
                    ? say('Copied!', '복사됨!', '¡Copiado!', 'Copié !', 'نُسخ!', '已复制！')
                    : say('Copy', '복사', 'Copiar', 'Copier', 'نسخ', '复制')}
                </button>
              </div>
              ...
              <p className="map-route__label">{say('길찾기 · Get there', '길찾기', 'Cómo llegar', 'Y aller', 'الوصول إليه', '怎么去')}</p>
            ...
            <footer className="provenance">
              <p className="provenance__title">{say('About this information', '이 정보에 대하여', 'Sobre esta información', 'À propos de ces informations', 'عن هذه المعلومات', '关于这些信息')}</p>
              <p>
                {say(
                  <>
              ...
              <dl className="provenance__list">
                <div>
                  <dt>{say('Location', '위치', 'Ubicación', 'Emplacement', 'الموقع', '位置')}</dt>
                  <dd>
                    {restaurant.coordinates.source}
                    {restaurant.address.precision === 'area' && say(' · address is area-level', ' · 주소는 구역 단위', ' · la dirección es a nivel de zona', " · l'adresse est au niveau du quartier", ' · العنوان على مستوى الحيّ', ' · 地址只到街区一级')}
                  </dd>
                </div>
                <div>
                  <dt>{say('Dietary', '식단', 'Dieta', 'Régime', 'النظام الغذائي', '饮食')}</dt>
                  <dd>
                    {dietFacts.length > 0
                      ? [...new Set(dietFacts.map(f => f.fact.source))].join(' · ')
                      : say('Not recorded', '기록 없음', 'Sin registrar', 'Non renseigné', 'غير مسجّل', '未记录')}
                  </dd>
                </div>
                <div>
                  <dt>{say('Last checked', '마지막 확인', 'Última comprobación', 'Dernière vérification', 'آخر تحقّق', '最近一次确认')}</dt>
                  <dd>{lastChecked ?? say('Never', '없음', 'Nunca', 'Jamais', 'أبدًا', '从未')}</dd>
                </div>
              </dl>
            </footer>
            ...
            <div className="transparency-log">
              {lastChecked && (
                <p>
                  {say('Last verified: ', '마지막 검증: ', 'Última verificación: ', 'Dernière vérification : ', 'آخر تحقّق: ', '最近一次确认：')}
              ...
              <p>
                {say('To suggest an edit, email ', '수정 제안은 이 주소로 보내주세요: ', 'Para sugerir una corrección, escribe a ', 'Pour proposer une correction, écrivez à ', 'لاقتراح تصحيح، اكتب إلى ', '想提修改意见，写信到 ')}
