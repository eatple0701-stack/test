      ...
      <header className="screen-head">
        ...
        <p className="screen-head__sub">
          {say(`${activePlaces.length} verified places, ${traditionalMarkets.length} markets.`,
            ...
            `${activePlaces.length} sitios verificados, ${traditionalMarkets.length} mercados.`, `${activePlaces.length} adresses vérifiées, ${traditionalMarkets.length} marchés.`, `${activePlaces.length} أماكن موثّقة، و${traditionalMarkets.length} أسواق.`, `${activePlaces.length} 处确认过的地点，${traditionalMarkets.length} 个市场。`, `確認済みの場所${activePlaces.length}か所、市場${traditionalMarkets.length}か所。`)}
        </p>
      </header>
