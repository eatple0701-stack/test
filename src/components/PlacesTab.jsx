...
const placeName = (place, say) => {
  ...
  return say(roman, inBrackets || roman, roman, roman);
};
    ...
    <section className="places-tab" aria-label="Places">
      <header className="screen-head">
        ...
        <h1 className="screen-head__title">
          {say('Places, markets and the city around them.', '식당과 시장, 그리고 그 주변의 도시.', 'Sitios, mercados y la ciudad alrededor.', 'Des adresses, des marchés et la ville autour.')}
        </h1>
        <p className="screen-head__sub">
          {say(`${activePlaces.length} verified places, ${traditionalMarkets.length} markets.`,
            ...
            `${activePlaces.length} sitios verificados, ${traditionalMarkets.length} mercados.`, `${activePlaces.length} adresses vérifiées, ${traditionalMarkets.length} marchés.`)}
        </p>
      </header>
      ...
      <div className="home-section">
        <div className="home-section__header">
          ...
          <h2>{say('Today\u2019s Korean Experience', '오늘의 한국 경험', 'La experiencia coreana de hoy', "L'expérience coréenne du jour")}</h2>
        </div>
        <button className="spotlight-card" onClick={() => openStory(todaysPick)}>
          ...
          <div className="spotlight-card__body">
            <span className="gather-card__tag">{say(todaysPick.zone, ZONE_KO[todaysPick.zone], ZONE_ES[todaysPick.zone], ZONE_FR[todaysPick.zone])}</span>
            <h3>{placeName(todaysPick, say)}</h3>
            <p className="spotlight-card__quote">&ldquo;{say(todaysPick.vibe, todaysPick.vibeKo, todaysPick.vibeEs, todaysPick.vibeFr)}&rdquo;</p>
            <span className="spotlight-card__cta">
              {say('Read the story', '이야기 읽기', 'Leer la historia', "Lire l'histoire")} <ChevronRightIcon size={14} />
            </span>
          </div>
        </button>
      </div>
      ...
      <div className="home-section">
        <div className="home-section__header">
          <h2>{say('Traditional Markets', '전통시장', 'Mercados tradicionales', 'Marchés traditionnels')}</h2>
            ...
            return (
              <div key={m.id} className={`market-card${visited ? ' is-visited' : ''}`}>
                <button className="market-card__hit" data-kr={m.nameKo} onClick={() => onExploreZone(m.zone)}>
                  ...
                  <h3>{say(m.name, m.nameKo, m.nameEs, m.nameFr)}</h3>
                  <p className="gather-card__meta"><MapPinIcon size={14} /> {say(m.zone, m.zoneKo, m.zoneEs, m.zoneFr)}</p>
                  <p className="gather-card__desc">{say(m.blurb, m.blurbKo, m.blurbEs, m.blurbFr)}</p>
                </button>
                {onToggleMarket && (
                  <button
                  ...
                  >
                    {visited
                      ? <><CheckIcon size={14} /> {say('Visited', '가봤어요', 'Visitado', 'Visité')}</>
                      : say('Mark visited', '가봤다고 표시', 'Marcar como visitado', 'Marquer comme visité')}
                  </button>
                )}
              </div>
            );
      ...
      <div className="home-section">
        <div className="home-section__header">
          <h2>{say('Popular with Travelers', '여행자들이 많이 찾는 곳', 'Los favoritos de los viajeros', 'Les préférés des voyageurs')}</h2>
        </div>
        <div className="home-scroll-row">
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Food Stories', '음식에 얽힌 이야기', 'Historias de comida', 'Histoires de cuisine')}</h2></div>
        <div className="home-scroll-row">
          {storyPicks.map(place => (
            <button key={place.id} className="gather-card" onClick={() => openStory(place)}>
              <span className="gather-card__tag">{say(place.zone, ZONE_KO[place.zone], ZONE_ES[place.zone], ZONE_FR[place.zone])}</span>
              <h3>{placeName(place, say)}</h3>
              <p className="gather-card__desc">&ldquo;{say(place.vibe, place.vibeKo, place.vibeEs, place.vibeFr)}&rdquo;</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Recommended Courses', '추천 코스', 'Rutas recomendadas', 'Parcours recommandés')}</h2></div>
        <div className="home-scroll-row">
          {courses.map(c => {
            ...
            return (
              <button key={c.id} className="course-card" onClick={() => first && onOpenRestaurant(first)}>
                ...
                <h3>{say(c.title, c.titleKo, c.titleEs, c.titleFr)}</h3>
                <p className="course-card__meta">{say(c.duration, c.durationKo, c.durationEs, c.durationFr)}</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Weekend Picks', '주말에 가볼 만한 곳', 'Para el fin de semana', 'Pour le week-end')}</h2></div>
        <div className="home-scroll-row">
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Local Hidden Gems', '현지인만 아는 곳', 'Joyas que solo conocen los locales', "Les adresses que seuls les gens d'ici connaissent")}</h2></div>
        <div className="home-scroll-row">
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Seasonal Foods', '제철 음식', 'Comida de temporada', 'Produits de saison')}</h2></div>
        <div className="home-scroll-row">
          {seasonalFoods.map(f => {
            ...
            return (
              <div key={f.id} className={`gather-card gather-card--static gather-card--kr${inSeason ? ' is-highlighted' : ''}`} data-kr={f.nameKo}>
                <span className="gather-card__tag">
                  {say(f.season, f.seasonKo, f.seasonEs, f.seasonFr)}
                  {inSeason ? say(' · In season now', ' · 지금이 제철', ' · ahora es temporada', " · c'est la saison") : ''}
                </span>
                <h3>{say(f.name, f.nameKo, f.nameEs, f.nameFr)} <span className="gather-card__kr">{f.nameKo}</span></h3>
                <p className="gather-card__desc">{say(f.blurb, f.blurbKo, f.blurbEs, f.blurbFr)}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Festival Picks', '축제', 'Festivales', 'Festivals')}</h2></div>
        <div className="home-scroll-row">
          {festivals.map(f => (
            <div key={f.id} className="gather-card gather-card--static gather-card--kr" data-kr={f.nameKo}>
              <span className="gather-card__tag">{say(f.when, f.whenKo, f.whenEs, f.whenFr)}</span>
              <h3>{say(f.name, f.nameKo, f.nameEs, f.nameFr)} <span className="gather-card__kr">{f.nameKo}</span></h3>
              <p className="gather-card__desc">{say(f.blurb, f.blurbKo, f.blurbEs, f.blurbFr)}</p>
            </div>
          ))}
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Korean Dining Culture', '한국의 식문화', 'La cultura de la mesa coreana', 'La culture de la table coréenne')}</h2></div>
        <div className="home-scroll-row">
          {CULTURE_CARDS.slice(0, 4).map((c, i) => (
            <button key={c.title} className="zone-card" style={{ flex: '0 0 180px' }}
              ...
              <h3>{say(c.title, c.titleKo, c.titleEs, c.titleFr)}</h3>
              <p>{say(c.desc, c.descKo, c.descEs, c.descFr)}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="home-section">
        <div className="home-section__header"><h2>{say('Recommended Neighborhoods', '동네 추천', 'Barrios recomendados', 'Quartiers recommandés')}</h2></div>
        <div className="zone-grid">
          {featuredZones.map(z => (
            <button key={z.name} className="zone-card" onClick={() => onExploreZone(z.name)}>
              <h3>{say(z.name, z.nameKo, z.nameEs, z.nameFr)}</h3>
              <p>{say(z.blurb, z.blurbKo, z.blurbEs, z.blurbFr)}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="home-section home-section--tight">
        ...
        >
          <span className="map-cta__title">{say('See it all on the map', '지도에서 한눈에 보기', 'Verlo todo en el mapa', 'Voir tout sur la carte')}</span>
          <span className="map-cta__body">
            {say('Every place above, plotted — filter by zone, diet or vibe.',
              '위의 모든 장소가 지도 위에 찍혀 있습니다. 지역, 식단, 분위기로 걸러 보세요.', 'Todos los sitios de arriba, en el mapa: filtra por zona, dieta o ambiente.', 'Toutes les adresses ci-dessus, sur la carte : filtrez par quartier, régime ou ambiance.')}
          </span>
          <span className="map-cta__link">
            {say('Open the map', '지도 열기', 'Abrir el mapa', 'Ouvrir la carte')} <ChevronRightIcon size={16} />
          </span>
        </button>
      </div>
