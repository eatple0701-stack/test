    ...
    return (
      <section className="sheet-page">
        <header className="sheet-page__head">
          ...
          <h1>{say('Table', '밥상', 'Mesa', 'Table', 'مائدة', '饭桌')}</h1>
        </header>
        <p className="tables-empty">
          {say('This table is no longer here.', '이 밥상은 사라졌어요.', 'Esta mesa ya no está aquí.', "Cette table n'est plus là.", 'لم تعد هذه المائدة هنا.', '这张饭桌已经不在了。')}
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="sheet-page table-detail" aria-label={`${menu.name} table`}>
      ...
      <div className="detail-hero">
        ...
        <p className="detail-hero__gloss">{say(menu.gloss, menu.glossKo, menu.glossEs, menu.glossFr, menu.glossAr, menu.glossZh)}</p>
        <p className="detail-hero__why">{say(menu.whyShared, menu.whySharedKo, menu.whySharedEs, menu.whySharedFr, menu.whySharedAr, menu.whySharedZh)}</p>
      </div>
      ...
      <div className="detail-block detail-block--facts">
            ...
            : (
              <>
                {say('Restaurant not decided yet — the table picks one together',
                  ...
                  'Restaurante aún sin decidir: lo elige la mesa entre todos', 'Restaurant pas encore décidé — la table le choisit ensemble', 'المطعم لم يُحدَّد بعد — تختاره المائدة معًا', '餐厅还没定 — 由这桌一起选')}
              </>
            )}
        </p>
        ...
        <button className="detail-share" translate="no" onClick={share}>
          {shared
            ? <><CheckIcon size={15} /> {say('Link copied', '링크 복사됨', 'Enlace copiado', 'Lien copié', 'نُسخ الرابط', '链接已复制')}</>
            : say('링크 보내기 · Send this table to someone', '링크 보내기', 'Enviar esta mesa a alguien', "Envoyer cette table à quelqu'un", 'أرسل هذه المائدة إلى أحد', '把这张饭桌发给谁')}
        </button>
        ...
        {!isCancelled(table) && !isPast(table) && (
          <>
            <button className="detail-share detail-calendar" translate="no" onClick={addToCalendar}>
              {say('캘린더에 추가 · Add to your calendar', '캘린더에 추가', 'Añadir a tu calendario', 'Ajouter à votre agenda', 'أضِفها إلى تقويمك', '加进你的日历')}
            </button>
            ...
            {gcalUrl && (
              <a className="detail-share detail-calendar--google" href={gcalUrl} target="_blank" rel="noopener noreferrer">
                {say('구글 캘린더로 추가 · Add with Google Calendar', '구글 캘린더로 추가', 'Añadir con Google Calendar', 'Ajouter avec Google Agenda', 'أضِفها بتقويم Google', '用 Google 日历添加')}
              </a>
            )}
      ...
      <div className="detail-block">
        <h3 className="detail-block__label">
          {say('What happens at the table', '식탁에서 벌어지는 일', 'Qué pasa en la mesa', 'Ce qui se passe à table', 'ما الذي يحدث على المائدة', '桌上会发生什么')}
        </h3>
        <p className="detail-block__body">{say(menu.howItWorks, menu.howItWorksKo, menu.howItWorksEs, menu.howItWorksFr, menu.howItWorksAr, menu.howItWorksZh)}</p>
        ...
        <button className="phrase-open" onClick={() => setPhrasesOpen(true)}>
          {say('식탁에서 · What to say at the table', '식탁에서',
            'Qué decir en la mesa', 'Quoi dire à table', 'ماذا تقول على المائدة', '在桌上该说什么')}
        </button>
      </div>
      ...
      {menu.culture && (
        <details className="detail-block culture-fold">
          <summary className="detail-block__label culture-fold__summary">
            {say('Why it is eaten together', '왜 함께 먹나', 'Por qué se come en compañía', 'Pourquoi on le mange ensemble', 'لماذا يُؤكل مع الناس', '为什么要一起吃')}
            <ChevronRightIcon size={14} />
          </summary>
          <p className="detail-culture">{say(menu.culture, menu.cultureKo, menu.cultureEs, menu.cultureFr, menu.cultureAr, menu.cultureZh)}</p>
          ...
          {theme && onOpenTheme && (
            <button className="detail-theme" onClick={() => onOpenTheme(theme.id)}>
              <span className="detail-theme__label">{say('Part of', '속한 이야기', 'Forma parte de', 'Fait partie de', 'جزء من', '属于')}</span>
      ...
      <div className="detail-block">
        <h3 className="detail-block__label">{say('언어 · What this table runs in', '언어', 'En qué idioma va esta mesa', 'La langue de cette table', 'لغة هذه المائدة', '这张饭桌用的语言')}</h3>
        {tableLanguages.length > 0 ? (
          <p className="lang-row">
            {tableLanguages.map(l => <span key={l} className="lang-chip">{l}</span>)}
          </p>
        ) : (
          <p className="lang-note">{say('The host did not say.', '호스트가 밝히지 않았어요.', 'El anfitrión no lo ha dicho.', "L'hôte ne l'a pas dit.", 'لم يذكر المضيف ذلك.', '主人没有说。')}</p>
        )}
        ...
        {fit === LANGUAGE_FIT.NONE && (
          <p className="lang-note lang-note--warn">
            {say('Nothing here matches what you speak. People manage — but bring a translation app, and the phrases below.',
              ...
              'Aquí no hay ningún idioma que hables. La gente se apaña, pero llévate una app de traducción y las frases de abajo.', "Aucune langue ici ne correspond à ce que vous parlez. On s'en sort — mais prenez une application de traduction, et les phrases ci-dessous.", 'لا لغة هنا تطابق ما تتحدّثه. يتدبّر الناس أمرهم — لكن خذ معك تطبيق ترجمة، والعبارات أدناه.', '这里没有你会说的语言。大家都能应付过来——不过带上一个翻译应用，还有下面这些句子。')}
          </p>
        )}
        {fit === LANGUAGE_FIT.MINE_UNSAID && (
          <p className="lang-note">
            {say('You have not said what you speak, so this cannot be compared. Profile.',
              ...
              'No has dicho qué idiomas hablas, así que no se puede comparar. Perfil.', "Vous n'avez pas dit quelles langues vous parlez, la comparaison est donc impossible. Profil.", 'لم تذكر اللغات التي تتحدّثها، فالمقارنة متعذّرة. الملف الشخصي.', '你还没填自己会说什么语言，所以没法比对。去资料里填。')}
          </p>
        )}
      </div>
      ...
      {host && (
        <div className="detail-block host-card">
          <h3 className="detail-block__label">{say('호스트 · Who is cooking this evening', '호스트', 'Quién organiza esta noche', 'Qui reçoit ce soir', 'من يستضيف هذه الأمسية', '今晚谁做东')}</h3>
      ...
      {guidesOffered.length > 0 && (
        <div className="detail-block">
          <h3 className="detail-block__label">{say('문화 가이드 · What the host will show you', '문화 가이드', 'Lo que te enseñará el anfitrión', "Ce que l'hôte vous montrera", 'ما الذي سيعرّفك به المضيف', '主人会带你看什么')}</h3>
          ...
          <p className="guide-list__note">
            {say('Said by the host when they opened this table, in their words.',
              ...
              'Lo dijo el anfitrión al abrir esta mesa, con sus palabras.', "Dit par l'hôte à l'ouverture de cette table, avec ses mots.", 'قاله المضيف عند فتح هذه المائدة، بكلماته.', '开这张饭桌时主人自己说的话。')}
          </p>
        </div>
      )}
        ...
        <div className="detail-block meeting-block">
          ...
          {table.chatUrl && (
            <a className="meeting-chat" translate="no" href={table.chatUrl} target="_blank" rel="noopener noreferrer">
              {say('오픈채팅 참여 · Join this table’s open chat', '오픈채팅 참여', 'Entrar en el chat abierto de esta mesa', 'Rejoindre le chat ouvert de cette table', 'ادخل المحادثة المفتوحة لهذه المائدة', '加入这张饭桌的开放聊天')}
            </a>
          )}
        </div>
      )}
      ...
      <div className="detail-block">
        <h3 className="detail-block__label">{say('Who is going', '누가 가나', 'Quién va', 'Qui vient', 'من سيأتي', '谁会来')}</h3>
        <ul className="who-list">
          <li className="who-row">
            <span className="who-row__dot" aria-hidden="true" />
            <span className="who-row__name">{table.hostName}</span>
            <span className="who-row__role">{say('host', '호스트', 'anfitrión', 'hôte', 'المضيف', '主人')}</span>
            ...
            {table.hostVerified && (
              <span className="who-row__verified">
                {say('인증 · verified', '인증', 'verificado', 'vérifié', 'موثّق', '已核实')}
            ...
            {!isHost && (
              justBlocked.has(table.hostId) ? (
                <span className="who-row__blocked">{say('Blocked', '차단됨', 'Bloqueado', 'Bloqué', 'محظور', '已拉黑')}</span>
              ) : (
                <button
                  className="who-row__block"
                  onClick={() => setConfirmBlock({ id: table.hostId, name: table.hostName, role: 'host' })}
                >
                  {say('Block', '차단', 'Bloquear', 'Bloquer', 'حظر', '拉黑')}
                </button>
              )
            )}
          </li>
          {signups.map(s => (
            <li key={s.id} className="who-row who-row--stacked">
              ...
              <span className="who-row__line">
                ...
                {isDeclined(s) && <span className="who-row__declined">{say('not this time', '이번엔 아니요', 'esta vez no', 'pas cette fois', 'ليس هذه المرة', '这次不了')}</span>}
              ...
              {isHost && !isCancelled(table) && isPending(s) && !hasLapsed(s, table) && (
                <span className="decide-row">
                  ...
                  >
                    {say('자리 드리기 · Give the seat', '자리 드리기', 'Dar el sitio', 'Donner la place', 'امنح المقعد', '把位子给他')}
                  </button>
                  <button
                  ...
                  >
                    {say('Not this time', '이번엔 아니요', 'Esta vez no', 'Pas cette fois', 'ليس هذه المرة', '这次不了')}
                  </button>
          ...
          <div className="cancel-confirm">
            ...
            <div className="cancel-confirm__row">
              <button className="cancel-confirm__no" onClick={() => setConfirmBlock(null)}>
                {say('Keep as is', '그대로 두기', 'Dejarlo como está', 'Laisser tel quel', 'اتركه كما هو', '保持原样')}
              </button>
      ...
      {mySignup ? (
        ...
        <div className="join-block">
          ...
          {isCancelled(table) ? null : !confirmCancel ? (
            <button className="join-leave" onClick={() => setConfirmCancel(true)}>
              {say('이 상 취소 · Call off this table', '이 상 취소', 'Cancelar esta mesa', 'Annuler cette table', 'ألغِ هذه المائدة', '取消这张饭桌')}
            </button>
          ) : (
            <div className="cancel-confirm">
              <p className="cancel-confirm__title">{say('Call this table off?', '이 밥상을 접을까요?', '¿Cancelar esta mesa?', 'Annuler cette table ?', 'أتلغي هذه المائدة؟', '要取消这张饭桌吗？')}</p>
              ...
              {affected.length > 0 ? (
                ...
                <p className="cancel-confirm__body">{say('Nobody has taken a seat, so nobody is affected.', '아직 아무도 자리를 잡지 않아서 영향받는 사람이 없습니다.', 'Nadie ha ocupado un sitio, así que no afecta a nadie.', "Personne n'a pris de place, donc personne n'est concerné.", 'لم يأخذ أحد مقعدًا، فلا أحد يتأثّر.', '还没有人占位子，所以不会影响到谁。')}</p>
              )}
              <div className="cancel-confirm__row">
                <button className="cancel-confirm__no" onClick={() => setConfirmCancel(false)}>
                  {say('Keep it', '그냥 두기', 'Mantenerla', 'La garder', 'أبقِها', '留着')}
                </button>
        ...
        <div className="join-block">
          ...
          {blocker === JOIN_BLOCK.FULL && (
            <p className="join-next">{say('Open the same dish yourself and fill your own table.', '같은 요리로 직접 상을 열고 그 자리를 채우세요.', 'Abre tú el mismo plato y llena tu propia mesa.', 'Ouvrez vous-même le même plat et remplissez votre table.', 'افتح الطبق نفسه بنفسك واملأ مائدتك.', '自己开一张同样这道菜的饭桌，把它坐满。')}</p>
          )}
        </div>
      ) : !isMember(auth) ? (
        ...
        <div className="join-block">
          ...
          <button className="form-submit" translate="no" onClick={() => onRequireAuth?.("join-table")}>
            {say(gateText('join-table').cta, null, gateText('join-table').ctaEs, gateText('join-table').ctaFr, gateText('join-table').ctaAr, gateText('join-table').ctaZh)}
          </button>
        </div>
        ...
        <div className="join-block">
          <h3 className="detail-block__label">{say('Before your first seat', '첫 자리를 잡기 전에', 'Antes de tu primer sitio', 'Avant votre première place', 'قبل مقعدك الأول', '在你的第一个位子之前')}</h3>
          <RulesConsent
            profile={profile}
        ...
        <div className="join-block">
          <h3 className="detail-block__label">{say('Ask for a seat', '자리 요청하기', 'Pedir sitio', 'Demander une place', 'اطلب مقعدًا', '申请一个位子')}</h3>
          ...
          {profileKnown && !editingIdentity ? (
            <p className="join-as">
              ...
              <button className="join-as__edit" onClick={() => setEditingIdentity(true)}>
                {say('Change', '수정', 'Cambiar', 'Changer', 'تغيير', '更改')}
              </button>
            </p>
          ) : (
            <>
              <label className="field">
                <span className="field__label">{say('Your name', '이름', 'Tu nombre', 'Votre nom', 'اسمك', '你的名字')}</span>
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={say('What to call you', '뭐라고 부를까요', 'Cómo llamarte', 'Comment vous appeler', 'بماذا نناديك', '怎么称呼你')} />
              </label>
              <label className="field">
                <span className="field__label">{say('Where you are from (optional)', '어디서 오셨는지 (선택)', 'De dónde eres (opcional)', "D'où vous venez (facultatif)", 'من أين أنت (اختياري)', '你从哪儿来（可选）')}</span>
                <input type="text" value={nationality} onChange={e => setNationality(e.target.value)} placeholder={say('Japan', '일본', 'Japón', 'Japon', 'اليابان', '日本')} />
              </label>
            </>
          )}
          ...
          {!noteOpen ? (
            <button className="join-note-open" onClick={() => setNoteOpen(true)}>
              {say('+ Anything the table should know?', '+ 밥상에서 알아야 할 게 있나요?', '+ ¿Algo que la mesa deba saber?', '+ Quelque chose que la table devrait savoir ?', '+ أشيء ينبغي أن تعرفه المائدة؟', '+ 有什么要让这桌知道的吗？')}
            </button>
          ) : (
            <label className="field">
              <span className="field__label">{say('Anything the table should know?', '밥상에서 알아야 할 게 있나요?', '¿Algo que la mesa deba saber?', 'Quelque chose que la table devrait savoir ?', 'أشيء ينبغي أن تعرفه المائدة؟', '有什么要让这桌知道的吗？')}</span>
              <textarea rows={2} value={note} onChange={e => setNote(e.target.value)} placeholder={say('No pork, and my Korean is about ten words.', '돼지고기는 못 먹고, 한국어는 열 단어쯤 합니다.', 'Sin cerdo, y mi coreano son unas diez palabras.', 'Pas de porc, et mon coréen tient en dix mots.', 'بلا لحم خنزير، وكوريّتي عشر كلمات تقريبًا.', '不吃猪肉，我的韩语大概就十个词。')} autoFocus />
            </label>
          )}
      ...
      {reportSent ? (
        ...
        <div className="report-panel">
          ...
          <div
            ...
            role="group"
            aria-label={say('Reason', '이유', 'Motivo', 'Motif', 'السبب', '原因')}
          >
          ...
          <label className={`field${reportProblems.some(p => p.field === 'note') ? ' is-bad' : ''}`}>
            <span className="field__label">{say('무슨 일이 있었나요? · What happened, in your words', '무슨 일이 있었나요?', 'Qué pasó, con tus palabras', "Ce qui s'est passé, avec vos mots", 'ما الذي حدث، بكلماتك', '发生了什么，用你自己的话')}</span>
            <textarea
          ...
          <div className="report-panel__row">
            <button className="cancel-confirm__no" onClick={() => setReportOpen(false)}>
              {say('Never mind', '취소', 'Déjalo', 'Laissez tomber', 'دع الأمر', '算了')}
            </button>
      ...
      <button className="safety-open" translate="no" onClick={() => setSafetyOpen(true)}>
        {say('도움이 필요하면 · Feeling unsafe or need help?', '도움이 필요하면', '¿No te sientes seguro o necesitas ayuda?', "Vous ne vous sentez pas en sécurité ou besoin d'aide ?", 'ألا تشعر بالأمان أو تحتاج مساعدة؟', '觉得不安全或需要帮助？')}
      </button>
      ...
      {onOpenTable && similar.length > 0 && (
        <div className="detail-block similar-block">
          <h3 className="detail-block__label">{say('이런 밥상은 어때요 · Other tables open now', '이런 밥상은 어때요', 'Otras mesas abiertas ahora', "D'autres tables ouvertes en ce moment", 'موائد أخرى مفتوحة الآن', '现在开着的其他饭桌')}</h3>
      ...
      {!isHost && !mySignup && !blocker && !isCancelled(table) && !isPast(table) && (
        <div className="detail-cta">
          ...
          >
            {say('자리 요청 · Take a seat', '자리 요청', 'Pedir sitio', 'Demander une place', 'اطلب مقعدًا', '申请一个位子')}
          </button>
        </div>
      )}
