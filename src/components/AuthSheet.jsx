...
export default function AuthSheet({ door, initialMode, profile, onProfileChange, onClose, onAuthed }) {
    ...
    <div className="auth-error" role="alert">
      ...
      {error.action === AUTH_ACTION.SIGNIN && (
        <button className="auth-error__go" onClick={() => { setMode('signin'); setError(null); }}>
          {say('로그인하기 · Go to sign in', '로그인하기', 'Ir a iniciar sesión', 'Aller à la connexion', 'الذهاب إلى تسجيل الدخول')}
        </button>
      )}
      {error.action === AUTH_ACTION.SIGNUP && (
        <button className="auth-error__go" onClick={() => { setMode('signup'); setError(null); }}>
          {say('회원가입하기 · Create an account', '회원가입하기', 'Crear una cuenta', 'Créer un compte', 'إنشاء حساب')}
        </button>
      )}
    </div>
  );
  ...
  return (
    <div className="auth-backdrop" role="dialog" aria-modal="true" aria-label={say('Sign in', '로그인', 'Entrar', 'Se connecter', 'تسجيل الدخول')}>
      <div className="auth-sheet">
        ...
        {(mode === 'signup-email') && (
          <button className="auth-back" onClick={() => { setMode('signup'); setError(null); }} aria-label={say('Back', '뒤로', 'Atrás', 'Retour', 'رجوع')}>
          ...
          <div className="auth-choose">
            ...
            <h2 className="auth-title">
              ...
              <span className="auth-title__en">
                {gate
                  ? say(gate.titleEn, null, gate.titleEs, gate.titleFr, gate.titleAr)
                  : say('Join Eatple', null, 'Únete a Eatple', 'Rejoindre Eatple', 'انضمّ إلى Eatple')}
              </span>
            </h2>
            {gate && <p className="auth-gate__body">{say(gate.body, gate.bodyKo, gate.bodyEs, gate.bodyFr, gate.bodyAr)}</p>}
            <button className="auth-google" onClick={google} translate="no">
              {say('Google로 계속 · Continue with Google', 'Google로 계속', 'Continuar con Google', 'Continuer avec Google', 'المتابعة بحساب Google')}
            </button>
            <p className="auth-or"><span>{say('또는 · or', '또는', 'o', 'ou', 'أو')}</span></p>
            ...
            <button className="auth-email-link" onClick={() => setMode('signup-email')} translate="no">
              {say('이메일로 가입 · Sign up with email', '이메일로 가입', 'Registrarse con correo', "S'inscrire par e-mail", 'التسجيل بالبريد')}
            </button>
            {errorLine}
            <p className="auth-foot">
              <span className="auth-foot__ask">{say('이미 계정이 있으신가요? · Already have an account?', '이미 계정이 있으신가요?', '¿Ya tienes cuenta?', 'Vous avez déjà un compte ?', 'ألديك حساب بالفعل؟')}</span>
              <button className="auth-foot__link" onClick={() => { setMode('signin'); setError(null); }}>
                {say('로그인 · Sign in', '로그인', 'Entrar', 'Se connecter', 'تسجيل الدخول')}
              </button>
            </p>
          </div>
        )}
        ...
        {mode === 'signup-email' && (
          <div className="auth-form">
            <h2 className="auth-title">
              ...
              <span className="auth-title__en">{say('Finish signing up', null, 'Termina el registro', "Terminer l'inscription", 'أكمل التسجيل')}</span>
            </h2>

            <Field id="email" bad={bad} problems={problems} markRef={markRef}
              label={say('이메일 · Email — this is your login ID', '이메일 — 로그인 ID입니다', 'Correo — es tu identificador', "E-mail — c'est votre identifiant", 'البريد الإلكتروني — وهو معرّف دخولك')}>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={say('you@example.com', 'you@example.com', 'tu@ejemplo.com', 'vous@exemple.com', 'you@example.com')} autoComplete="email" />
            </Field>
            <Field id="password" bad={bad} problems={problems} markRef={markRef}
              label={say('비밀번호 · Password', '비밀번호', 'Contraseña', 'Mot de passe', 'كلمة المرور')}>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={say('8+ characters', '8자 이상', '8 caracteres o más', '8 caractères ou plus', '8 أحرف فأكثر')} autoComplete="new-password" />
            </Field>
            <Field id="name" bad={bad} problems={problems} markRef={markRef}
              label={say('이름 · Name — what a table calls you', '이름 — 밥상에서 불릴 이름', 'Nombre — cómo te llamará una mesa', 'Nom — comment une table vous appellera', 'الاسم — ما تناديك به المائدة')}>
            ...
            <Field id="phone" bad={bad} problems={problems} markRef={markRef}
              label={say('전화번호 · Phone', '전화번호', 'Teléfono', 'Téléphone', 'رقم الهاتف')}>
            ...
            <Field id="birthdate" bad={bad} problems={problems} markRef={markRef}
              label={say('생년월일 · Date of birth', '생년월일', 'Fecha de nacimiento', 'Date de naissance', 'تاريخ الميلاد')}>
        ...
        {mode === 'signin' && (
          <div className="auth-form">
            <h2 className="auth-title">
              ...
              <span className="auth-title__en">{say('Sign in', null, 'Entrar', 'Se connecter', 'تسجيل الدخول')}</span>
            </h2>
            <button className="auth-google" onClick={google} translate="no">
              {say('Google로 계속 · Continue with Google', 'Google로 계속', 'Continuar con Google', 'Continuer avec Google', 'المتابعة بحساب Google')}
            </button>
            ...
            <Field id="email" bad={signinBad} problems={validateSignin({ email, password })}
              label={say('이메일 · Email', '이메일', 'Correo', 'E-mail', 'البريد الإلكتروني')}>
        ...
        {mode === 'details' && (
          <div className="auth-form">
            ...
            <h2 className="form-label">{say('거의 다 됐어요 · Almost there', '거의 다 됐어요', 'Ya casi está', 'On y est presque', 'أوشكت على الانتهاء')}</h2>
            <p className="auth-note">
        ...
        {mode === 'profile' && (
          <div className="auth-form">
            ...
            <h2 className="form-label">{say('프로필 · Your profile', '프로필', 'Tu perfil', 'Votre profil', 'ملفك')}</h2>
            <p className="auth-note">
            ...
            <button className="auth-primary" onClick={() => setMode('avatar')}>
              {say('계속 · Continue', '계속', 'Continuar', 'Continuer', 'متابعة')}
            </button>
          </div>
        )}
        ...
        {mode === 'avatar' && (
          <div className="auth-form auth-avatar">
            <h2 className="form-label">{say('프로필 사진 · A photo to be recognised by', '프로필 사진', 'Una foto para reconocerte', 'Une photo pour vous reconnaître', 'صورة يُعرَف بها وجهك')}</h2>
            <p className="auth-note">
            ...
            <button className="auth-switch" onClick={finish} disabled={busy}>
              {say('나중에 · Later — you can add one any time', '나중에 — 언제든 추가할 수 있어요', 'Más tarde — puedes añadirla cuando quieras', 'Plus tard — vous pourrez en ajouter une à tout moment', 'لاحقًا — تستطيع إضافتها في أي وقت')}
            </button>
          </div>
        )}
      </div>
