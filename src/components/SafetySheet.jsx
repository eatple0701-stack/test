...
export default function SafetySheet({ onClose }) {
  const say = useText();
  return (
    <div className="phrase-sheet" role="dialog" aria-label={say('Safety', '안전', 'Seguridad', 'Sécurité', 'الأمان', '安全', '安全')}>
      <header className="sheet-page__head">
        <button className="sheet-page__back" onClick={onClose} aria-label="Close">
          <ChevronLeftIcon size={20} />
        </button>
        <h1>{say('도움 · Getting help', '도움', 'Cómo pedir ayuda', "Obtenir de l'aide", 'طلب المساعدة', '需要帮助时', '助けが必要なとき')}</h1>
      </header>
      ...
      <div className="safety">
        <a className="safety__call" href="tel:112">
          ...
          <span className="safety__call-label">{say('Police', '경찰', 'Policía', 'Police', 'الشرطة', '警察', '警察')}</span>
        </a>
        <a className="safety__call" href="tel:119">
          <span className="safety__call-num">119</span>
          <span className="safety__call-label">{say('Fire · ambulance', '화재 · 구급', 'Bomberos y ambulancia', 'Pompiers et ambulance', 'الإطفاء والإسعاف', '火警与急救', '消防・救急')}</span>
        </a>
        <a className="safety__call safety__call--wide" href="tel:1330">
          <span className="safety__call-num">1330</span>
          <span className="safety__call-label">{say('Korea Travel Helpline · 24h, English', '관광통역안내 · 24시간', 'Línea de ayuda al viajero · 24 h, en inglés', 'Assistance aux voyageurs · 24 h, en anglais', 'مساعدة المسافرين · على مدار الساعة، بالإنجليزية', '旅游咨询热线 · 24小时，英语', '旅行者向け案内 · 24時間、英語')}</span>
        </a>
      </div>
      ...
      <div className="phrase-list">
        ...
        {reportingConfigured() ? (
          <a className="safety__report" href={REPORT_CHANNEL.href} target="_blank" rel="noreferrer">
            {say('Report to ', '신고 보내기: ', 'Informar a ', 'Signaler à ', 'أبلِغ ', '举报给 ', '報告先 ')}{REPORT_CHANNEL.label}
          </a>
        ) : (
