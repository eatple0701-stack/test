    ...
    <div className="match-modal-backdrop" role="dialog" onClick={onClose} style={{ zIndex: 9999 }}>
      <div className="match-modal" onClick={e => e.stopPropagation()} style={{ padding: '40px 20px', minHeight: 300, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button className="btn-secondary" onClick={prev} style={{flex: 1}}>{say('Previous', '이전', 'Anterior', 'Précédent', 'السابق')}</button>
          <button className="btn-primary" onClick={next} style={{flex: 1}}>{say('Next Card', '다음 카드', 'Siguiente ficha', 'Fiche suivante', 'البطاقة التالية')}</button>
        </div>
