...
export default function HostBrief({ profile }) {
  ...
  return (
    <section className="host-brief" aria-label={say('Before you set a table', '상을 차리기 전에', 'Antes de poner una mesa', 'Avant de dresser une table', 'قبل أن تمدّ مائدة', '摆一张饭桌之前', '食卓を整える前に')}>
      <p className="host-brief__kr">밥상을 열기 전에</p>
      <h2 className="host-brief__title">{say('Before you set a table', '상을 차리기 전에', 'Antes de poner una mesa', 'Avant de dresser une table', 'قبل أن تمدّ مائدة', '摆一张饭桌之前', '食卓を整える前に')}</h2>
        ...
        {POINTS.map(p => (
          <div key={p.kr} className="host-brief__item">
            ...
            <dd className="host-brief__def">
              <span className="host-brief__kr">{p.ko}</span>
