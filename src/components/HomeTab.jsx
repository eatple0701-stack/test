      ...
      <div className="home-section">
        <div className="stack-head">
          ...
          <h2 className="stack-head__title">
            {say('Seven questions about how Korea eats', '한국이 어떻게 먹는지에 대한 일곱 가지 질문', 'Siete preguntas sobre cómo come Corea')}
          </h2>
          <p className="stack-head__sub">
            {say('Each one is a culture you can walk into.', '하나하나가 걸어 들어가 볼 수 있는 문화입니다.', 'Cada una es una cultura en la que puedes entrar.')}
          </p>
        </div>
      ...
      <div className="home-section home-section--tight">
        <div className="surprise-row">
          <button className="surprise-btn" onClick={() => { setCultureStart(0); setShowCulture(true); }}>
            ...
            <span className="surprise-btn__label">{say('Culture Cards', '문화 카드', 'Fichas de cultura')}</span>
          </button>
          <button className="surprise-btn" onClick={() => setShowRoulette(true)}>
            <span className="surprise-btn__kr">오늘 뭐 먹지</span>
            <span className="surprise-btn__label">{say('Pick a dish for me', '골라주세요', 'Elige un plato por mí')}</span>
          </button>
        </div>
      </div>
