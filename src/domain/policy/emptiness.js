...
export function emptyText(reason, { otherDays = false, locale = 'both' } = {}) {
  ...
  switch (reason) {
    case EMPTY.GENDER:
      return {
        title: pick('Nobody has said yet.', '아직 아무도 밝히지 않았어요.', 'Todavía no lo ha indicado nadie.', "Personne ne l'a encore indiqué.", 'لم يذكره أحد بعد.'),
        body: pick(
    ...
    case EMPTY.DAY:
      return {
        title: pick('Nothing on that day yet.', '그날은 아직 아무것도 없어요.', 'Ese día todavía no hay nada.', "Rien ce jour-là pour l'instant.", 'لا شيء في ذلك اليوم بعد.'),
        body: otherDays
          ? pick(
    ...
    case EMPTY.DISH:
      return {
        title: pick('No table for this dish yet.', '이 요리로 열린 밥상이 아직 없어요.', 'Aún no hay mesa para este plato.', 'Pas encore de table pour ce plat.', 'لا مائدة لهذا الطبق بعد.'),
        body: pick(
    ...
    case EMPTY.NONE:
      ...
      return {
        title: pick('No tables open this week.', '이번 주에 열린 밥상이 없어요.', 'Esta semana no hay mesas abiertas.', "Aucune table ouverte cette semaine.", 'لا موائد مفتوحة هذا الأسبوع.'),
        body: pick(
